...
function ValidarCampos(mode)
{
    ...
    //console.log("Valor de h_tienda:", parent.pantalla.document.albaranes_pro.h_tienda.value);
    //Validación de campo requerido para centro de costos - DVV
    if (parent.pantalla.document.albaranes_pro.tienda != undefined) {
        if (parent.pantalla.document.albaranes_pro.tienda.value == "") {
            window.alert("Centro de Costos es un campo obligatorio.");
            return false;
        }
    }
    ...
    var valid = (parent.pantalla.document.albaranes_pro.hora.value.search(/^\d{2}:\d{2}:\d{2}$/) != -1) &&
            (parent.pantalla.document.albaranes_pro.hora.value.substr(0,2) >= 0 && parent.pantalla.document.albaranes_pro.hora.value.substr(0,2) <= 24) &&
            (parent.pantalla.document.albaranes_pro.hora.value.substr(3,2) >= 0 && parent.pantalla.document.albaranes_pro.hora.value.substr(3,2) <= 59) &&
        	(parent.pantalla.document.albaranes_pro.hora.value.substr(6, 2) >= 0 && parent.pantalla.document.albaranes_pro.hora.value.substr(6, 2) <= 59);
    if (!(valid)) {
        window.alert("Error en el formato de la hora. Debe ser hh:mm:ss"); //"Error en el formato de la hora. Debe ser hh:mm:ss" btorreme 02/04/2024 - fix literal
        return false;
    }
...
function Buscar()
{
    ...
    SearchPage("SHLMX_deliveryNote_pro_lsearch.asp?mode=search&campo=" + document.opciones.campos.value +
	...
	switch (mode) {
		case "browse":
			switch (pulsado) {
				case "add": //Nuevo registro
					...
					parent.pantalla.document.albaranes_pro.action="SHLMX_albaranes_pro.asp?mode=" + pulsado;
					parent.pantalla.document.albaranes_pro.submit();
					document.location="SHLMX_albaranes_pro_bt.asp?mode=" + pulsado;
					break;
                ...
                case "edit": //Editar registro				  
				    if (parent.pantalla.document.albaranes_pro.h_nbalance.value>"") alert("<%=LitMsgModifAlbaranBalance%>");
						...
						parent.pantalla.document.albaranes_pro.action="SHLMX_albaranes_pro.asp?nalbaran=" + parent.pantalla.document.albaranes_pro.h_nalbaran.value +
						"&mode=" + pulsado;
						parent.pantalla.document.albaranes_pro.submit();
						document.location="SHLMX_albaranes_pro_bt.asp?mode=" + pulsado;
					}
				...
				case "edit2": //Editar Detalles del documento
					if (parent.pantalla.document.albaranes_pro.h_nbalance.value>"") alert("<%=LitMsgModifAlbaranBalance%>");
						...
						document.location.ref="SHLMX_albaranes_pro_bt.asp?mode=browse";
						pagina="../../../central.asp?pag1=custom/ShellMexico/compras/albaranes_prodet.asp&ndoc=" + parent.pantalla.document.albaranes_pro.h_nalbaran.value +
						"&nproveedor=" + parent.pantalla.document.albaranes_pro.h_nproveedor.value + "&mode=browse&pag2=custom/ShellMexico/compras/albaranes_prodet_bt.asp&titulo=<%=LitDetAlb%> " + parent.pantalla.document.albaranes_pro.h_nalbaran.value;
				...
				case "delete": //Eliminar registro
					if (parent.pantalla.document.albaranes_pro.h_nbalance.value>"")alert("<%=LitMsgEliminarAlbaranBalance%>");
						...
						if (parent.pantalla.document.albaranes_pro.h_nfactura.value=="NO") {
							...
							"&mode=" + pulsado;
							parent.pantalla.document.albaranes_pro.submit();
							document.location="SHLMX_albaranes_pro_bt.asp?mode=browse";
						}
		...
		case "add":
			switch (pulsado) {
				case "save": //Guardar registro
					if (ValidarCampos(mode)) {
						parent.pantalla.document.albaranes_pro.action="SHLMX_albaranes_pro.asp?mode=first_save";
						parent.pantalla.document.albaranes_pro.submit();
						document.location="SHLMX_albaranes_pro_bt.asp?mode=browse";
					}
					break;
				...
				case "cancel": //Cancelar edicion
					...
					parent.pantalla.document.albaranes_pro.action="SHLMX_albaranes_pro.asp?mode=add&viene=cancelar";
					parent.pantalla.document.albaranes_pro.submit();
					document.location="SHLMX_albaranes_pro_bt.asp?mode=add";
					break;
			}
			break;
	}

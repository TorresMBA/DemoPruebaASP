...
<!--#include file="../../../utilities_pmpk.inc" -->
...
<!--#include file="SHLMX_albaranes_pro.inc" -->
<!--#include file="../SHLMX_varios.inc" -->

<!--#include file="../../../compras/compras.inc" -->
<!--#include file="../../../compras/pedbis_pro.inc" -->
...
<!--#include file="SHLMX_albaranes_proActionDrop.inc" -->
<!--#include file="SHLMX_albaranes_pro_linkextra.inc" -->
...
'DVV: verificar permisos de usuario y guardar en session
...
%><script language="javascript" type="text/javascript">
      function addVencimiento(nalbaran) {
          ...
          fr_Vencimientos.document.vencimientos_pro_config.action = "../../../compras/vencimientos_pro_config.asp?mode=first_save";
      ...
      // Guardamos el value de tienda en el input hidden para validar campo requerido - DVV
      ...
      function TraerProveedor(mode) {
          ...
          cambiar_cliente = 0; // valor anterior vacío ""

          //DVV: no modifica la serie
          //if (confirm("<%=LitCambiarSeriePuedCamPro%>")) cambiar_cliente = 1;
          //else cambiar_cliente = 0;

          document.location.href = "SHLMX_albaranes_pro.asp?nproveedor=" + prov_old + "&mode=" + mode +
              ...
              <%if si_tiene_modulo_proyectos<>0 then%>
                  ...
                  <%end if%>
                      ...
                      "&s=" + document.albaranes_pro.s.value + "&almacenSerie=<%=almacenSerie %>&almacenTPV=<%=almacenTPV %>";
      }
      ...
      function Navegar(destino, origen) {
          document.albaranes_pro.action = "SHLMX_albaranes_pro.asp?nalbaran=" + origen + "&donde=" + destino + "&mode=search" + "&almacenSerie=<%=almacenSerie %>&almacenTPV=<%=almacenTPV %>";
          document.albaranes_pro.submit();
      }

      //***************************************************************************
      function Precios() {
          console.log("Ingresó a funcion de pro Precios");
          if (isNaN(document.albaranes_pro.dto1.value.replace(",", ".")) || isNaN(document.albaranes_pro.dto2.value.replace(",", ".")) || isNaN(document.albaranes_pro.rf.value.replace(",", ".")) || isNaN(document.albaranes_pro.irpf.value.replace(",", ".")))
              ...
              // DVV
              //console.log("dto1", dto1SinComas);
              //console.log("dto2", dto2SinComas);
              //console.log("dtoTotal", dtoTotal);
              //console.log("total_descuento.value", document.albaranes_pro.total_descuento.value);
      ...
      function CreaPedido(albaran) {
          document.location = "SHLMX_albaranes_pro.asp?nalbaran=" + albaran + "&mode=browse&viene=" + document.albaranes_pro.viene.value + "&seriepedido=" + document.albaranes_pro.seriePB.value + "&almacenSerie=<%=almacenSerie %>&almacenTPV=<%=almacenTPV %>";
          parent.botones.document.location = "SHLMX_albaranes_pro_bt.asp?mode=browse";
      }

      //****************************************************************************
      function CancelaPedido(albaran) {
          document.albaranes_pro.action = "SHLMX_albaranes_pro.asp?nalbaran=" + albaran + "&mode=browse" + "&almacenSerie=<%=almacenSerie %>&almacenTPV=<%=almacenTPV %>";
          document.albaranes_pro.submit();
          parent.botones.document.location = "SHLMX_albaranes_pro_bt.asp?mode=browse";
      }
      ...
      function Acaja(nalbaran) {
          ...
          if (document.albaranes_pro.ncaja.value == "") alert("<%=LitMsgCajaNoNulo%>");
              ...
              if (document.albaranes_pro.i_pago.value == "") alert("<%=LitMsgTipoPagoNoNulo%>");
                  ...
                  fr_PagosCuenta.document.albaranes_propagos.action = "./SHLMX_albaranes_propagos.asp?mode=acaja&ndoc=" + nalbaran + "&impcaja=" + document.albaranes_pro.impcaja.value + "&i_pago=" + document.albaranes_pro.i_pago.value + "&ncaja=" + document.albaranes_pro.ncaja.value;
      ...
      function addConcepto(nalbaran) {
          ...
          fr_Conceptos.document.albaranes_procon.action = "./SHLMX_albaranes_procon.asp?mode=first_save";
      ...
      function addPago(nalbaran) {
          ...
          //Recargar el submarco de pagos a cuenta
          fr_PagosCuenta.document.albaranes_propagos.action = "./SHLMX_albaranes_propagos.asp?mode=first_save";
          fr_PagosCuenta.document.albaranes_propagos.submit();
          //Limpiar los campos del formulario
          var hoy = new Date();
      ...
      function ImporteDetalle() {
          ...
          if (isNaN(document.albaranes_pro.cantidad.value.replace(",", ".")) || isNaN(document.albaranes_pro.descuento.value.replace(",", ".")) || isNaN(document.albaranes_pro.pvp.value.replace(",", ".")))
              ...
              pvpSinComas = document.albaranes_prodetins.h_costpr.value.replace(",", ".");//MACE - Coste sin impuestos por Proveedor
              iepsuno = document.albaranes_prodetins.h_iepsmon.value.replace(",", ".");//MACE - IEPS $ por Referencia

              cantidadSinComas = document.albaranes_pro.cantidad.value.replace(",", ".");
              dtoSinComas = document.albaranes_prodet.descuento.value.replace(",", ".");
              dto2SinComas = document.albaranes_prodet.descuento2.value.replace(",", ".");
               ...
               pvpieps=parseFloat(pvpcant)+parseFloat(iepsuno);
		        ...
		        importe = parseFloat(pelas_descuento2).toFixed(<%=NdecDiAlbaran %>);
              ...
              document.albaranes_prodet.descuento2.value = dto2SinComas;
      ...
      function MasDet(sentido, lote, firstReg, lastReg, campo, criterio, texto, firstRegAll, lastRegAll) {
          fr_Detalles.document.albaranes_prodet.action = "SHLMX_albaranes_prodet.asp?mode=browse&sentido=" + sentido + "&lote=" + lote + "&campo=" + campo + "&criterio=" + criterio + "&texto=" + texto + "&firstReg=" + firstReg + "&lastReg=" + lastReg + "&firstRegAll=" + firstRegAll + "&lastRegAll=" + lastRegAll + "&almacenSerie=<%=almacenSerie %>&almacenTPV=<%=almacenTPV %>";
	  ...
	  //INICIO - Validar calendario para mostrar sólo fecha actual
	  function DrawCalendarJQuery_Today(idInput, regional) {
        jQuery(function () {
            jQuery("input[name=" + idInput + "]").datepicker({
                showOn: "button",
                buttonImage: "/lib/estilos/HUBBLE/images/calendario.png",
            ...
            jQuery(".selector").datepicker({ buttonText: 'Choose' });
        });
    }
	
	//Validar fecha del input
	function validarFechaAlbaran(object){
		var today = new Date();
		var dd = String(today.getDate()).padStart(2, '0');
		var mm = String(today.getMonth() + 1).padStart(2, '0');
		var yyyy = today.getFullYear();
		
		var fecha_input = $(object).val();

		today = dd + '/' + mm + '/' + yyyy;
		if(fecha_input < today || fecha_input > today){
			alert("No puede ingresar una fecha menor o mayor a la fecha actual");
			$(object).val(today);
		}
	}
	//FIN - Validar calendario para mostrar sólo fecha actual
</script>
<script language="javascript" type="text/javascript">
...
'******************************************************************************
'Guardar & Actualiza los datos del registro cuando se pulsa el botón de 'GUARDAR'
sub GuardarRegistro(nalbaran,nserie)
	...
	if nalbaran="" then'Si es vacio crea un nuevo registro
    ...
    'DVV - verificar si el registro proviene de edición y de usuario no admin
    ...
    'SRH INICIO evolutivo añadir hora
	...
	'Inicio - Fix: se registra la fecha ingresada en el campo "fcreacion" btorreme 27/07/2023
	rst("fcreacion") = fechaHora
	'Fin - btorreme 27/07/2023
	
    'SRH FIN evolutivo añadir hora
		...
		if cambio_proveedor=true then
			...
			while not rstAux.eof
				...
				if not rstMiProveer.eof then
                    ...
                    'importe_imp	= null_z(d_sum("importe","detalles_alb_pro","nalbaran='" & nalbaran & "'",session("dsn_cliente")))'MACE
...
sub BorrarRegistro(nalbaran, nalbaran_pro)
    ...
    if noborrar then	    
	     %><script language="javascript" type="text/javascript">
               ...
               document.albaranes_pro.action = "SHLMX_albaranes_pro.asp?nalbaran=<%=nalbaran%>&mode=browse&almacenSerie=<%=almacenSerie %>&almacenTPV=<%=almacenTPV %>";
               document.albaranes_pro.submit();
               parent.botones.document.location = "SHLMX_albaranes_pro_bt.asp?mode=browse";
	     </script><%
	else
        'API - BORRAR DOCUMENTO - MACE
         if configPMPK = true then
          'Obtener FCREACION Y FECHANEGOCIO del albaran
              set rstFechas = Server.CreateObject("ADODB.Recordset")
              fcreacion=""
              fechanegocio=""
              set connFechas = Server.CreateObject("ADODB.Connection")
              set commandFechas =  Server.CreateObject("ADODB.Command")
              connFechas.open session("dsn_cliente")
              commandFechas.ActiveConnection =connFechas
              commandFechas.CommandTimeout = 0
              commandFechas.CommandText="PMPK_GET_DATE_BY_TYPE_DOCUMENT"
              commandFechas.CommandType = adCmdStoredProc 'Procedimiento Almacenado
              commandFechas.Parameters.Append commandFechas.CreateParameter("@PARAM_IN_NCOMPANY",adVarChar,adParamInput,5,session("ncliente"))
              commandFechas.Parameters.Append commandFechas.CreateParameter("@PARAM_IN_NDOCUMENT",adVarChar,adParamInput,20,nalbaran)
              commandFechas.Parameters.Append commandFechas.CreateParameter("@PARAM_IN_DOCUMENT_TYPE",adVarChar,adParamInput,30,"ALBARAN_PRO")
                    
	          rstFechas.Open commandFechas, , adOpenKeyset, adLockOptimistic
                      while not rstFechas.eof
                          fcreacion = rstFechas("FCREACION")
                          fechanegocio = rstFechas("FECHANEGOCIO")
                          rstFechas.movenext
                      wend
              rstFechas.Close
              connFechas.close
              set commandFechas=nothing
              set connFechas=nothing

              Dim detitem() 
              ReDim detitem(-1)
              Dim detReferenciaOld() 
              ReDim detReferenciaOld(-1)
              Dim detTiendaOld() 
              ReDim detTiendaOld(-1)
              Dim detCategoriaOld() 
              ReDim detCategoriaOld(-1)
              Dim detAlmacenOld() 
              ReDim detAlmacenOld(-1)
              Dim detCantidadOld() 
              ReDim detCantidadOld(-1)
              Dim detPvpOld() 
              ReDim detPvpOld(-1)
              Dim detTipoProductoOld() 
              ReDim detTipoProductoOld(-1)
          
              total = 0

             'Obtener Detalle del Albarán del albaran
              set rstDetalle = Server.CreateObject("ADODB.Recordset")
              almacen=""
              referencia=""
              categoria=""
              tienda=""
              ...
              cantidad=0
              pvp=0
              set connDetalle = Server.CreateObject("ADODB.Connection")
              set commandDetalle =  Server.CreateObject("ADODB.Command")
              connDetalle.open session("dsn_cliente")
              commandDetalle.ActiveConnection =connDetalle
              commandDetalle.CommandTimeout = 0
              commandDetalle.CommandText="PMPK_GET_DETAIL_DOCUMENT"
              commandDetalle.CommandType = adCmdStoredProc 'Procedimiento Almacenado
              commandDetalle.Parameters.Append commandDetalle.CreateParameter("@PARAM_IN_NCOMPANY",adVarChar,adParamInput,5,session("ncliente"))
              commandDetalle.Parameters.Append commandDetalle.CreateParameter("@PARAM_IN_NDOCUMENT",adVarChar,adParamInput,20,nalbaran)
              commandDetalle.Parameters.Append commandDetalle.CreateParameter("@PARAM_IN_DOCUMENT_TYPE",adVarChar,adParamInput,30,"ALBARAN_PRO")
                
	          rstDetalle.Open commandDetalle, , adOpenKeyset, adLockOptimistic
                while not rstDetalle.eof
                    almacen = rstDetalle("ALMACEN")
                    referencia = rstDetalle("REFERENCIA")
                    categoria = rstDetalle("CATEGORIA")
                    tienda = rstDetalle("TIENDA")
                    cantidad = rstDetalle("CANTIDAD")
                    pvp = rstDetalle("PVP")
                    item = rstDetalle("ITEM")
                      
                    pvp_replace = Replace(pvp,",",".")
                    
                    ReDim Preserve detAlmacenOld(UBound(detAlmacenOld) + 1)
                    ReDim Preserve detReferenciaOld(UBound(detReferenciaOld) + 1)
                    ReDim Preserve detCategoriaOld(UBound(detCategoriaOld) + 1)
                    ReDim Preserve detTiendaOld(UBound(detTiendaOld) + 1)
                    ReDim Preserve detCantidadOld(UBound(detCantidadOld) + 1)
                    ReDim Preserve detPvpOld(UBound(detPvpOld) + 1)
                    ReDim Preserve detitem(UBound(detitem) + 1)
                    ReDim Preserve detTipoProductoOld(UBound(detTipoProductoOld) + 1)

                    detAlmacenOld(UBound(detAlmacenOld)) = almacen
                    detReferenciaOld(UBound(detReferenciaOld)) = referencia
                    detCategoriaOld(UBound(detCategoriaOld)) = categoria
                    detTiendaOld(UBound(detTiendaOld)) = tienda
                    detCantidadOld(UBound(detCantidadOld)) = cantidad
                    detPvpOld(UBound(detPvpOld)) = pvp_replace
                    detTipoProductoOld(UBound(detTipoProductoOld)) = GetArticleType(referencia)
                    detitem(UBound(detitem)) = item
              ...
              rstDetalle.Close
              connDetalle.close
              set commandDetalle=nothing
              set connDetalle=nothing

            company = session("ncliente")
            user = session("usuario")
            codTypeOperation = "001"
            action = "DELETED"
            documentType = "ALBARAN_PRO"
            ndocumento = nalbaran
            documentnumber = nalbaran_pro
            ...
            ReDim detPmp(-1)
            Dim detAlmacenNew() 
            ReDim detAlmacenNew(-1)
            Dim detReferenciaNew() 
            ReDim detReferenciaNew(-1)
            Dim detCategoriaNew() 
            ReDim detCategoriaNew(-1)
            Dim detTiendaNew() 
            ReDim detTiendaNew(-1)
            Dim detCantidadNew() 
            ReDim detCantidadNew(-1)
            Dim detPvpNew() 
            ReDim detPvpNew(-1)
            Dim detStockUpd() 
            ReDim detStockUpd(-1)
            Dim detTipoProducto() 
            ReDim detTipoProducto(-1)
            Dim detAlmacenDest() 
            ReDim detAlmacenDest(-1)
            Dim detStockOtherTanks()
            ReDim detStockOtherTanks(-1)
            Dim detStockOtherTanksUpd()
            ReDim detStockOtherTanksUpd(-1)
            Dim detTipoProductoNew() 
            ReDim detTipoProductoNew(-1)

            Dim detstockdest()
            ReDim detstockdest(-1)
            Dim detpmpdest()
            ReDim detpmpdest(-1)

          For k=0 To total - 1
             almacen = detAlmacenOld(k)
             referencia = detReferenciaOld(k)

             'Obtener STOCK de la tabla ALMACENAR
              stock = GetStock(referencia, almacen)
              stock_replace = Replace(stock,",",".")
              ReDim Preserve detStock(UBound(detStock) + 1)
              detStock(UBound(detStock)) = stock_replace

             'Obtener PMP de la tabla ALMACENAR
              pmp = GetPmp(referencia, almacen)
              pmp_replace = Replace(pmp,",",".")
              ReDim Preserve detPmp(UBound(detPmp) + 1)
              detPmp(UBound(detPmp)) = pmp_replace
	    ...
	    if TieneFactura="NO" then
		   ...
		   <script language="javascript" type="text/javascript">
               ...
               document.location = "SHLMX_albaranes_pro.asp?nalbaran=<%=nalbaran%>&mode=browse" + "&almacenSerie=<%=almacenSerie %>&almacenTPV=<%=almacenTPV %>";
               parent.botones.document.location = "SHLMX_albaranes_pro_bt.asp?mode=browse";
		    </script>
	    <%end if

        'PMPK-API Generamos el Json y llamamos a la API
        if configPMPK = true then
            'Llamada función GetLoadWriteJson() 23/10/2020 MACE
            param_json = GetLoadWriteJsonDetailMassive(company, user, detAlmacenOld, detAlmacenNew, detAlmacenDest, ndocumento, fcreacion, fechanegocio, documentnumber, detReferenciaOld, detReferenciaNew, detCantidadOld, detCantidadNew, detPvpOld, detPvpNew, detCategoriaOld, detCategoriaNew, detTiendaOld, detTiendaNew, detStock, detPmp, detStockUpd, detStockOtherTanks, detStockOtherTanksUpd, codTypeOperation, documentType, action, total, detTipoProductoOld, detTipoProductoNew, detitem, detstockdest, detpmpdest)
           
          'Insertar En La Tabla Transaccional  - Obtener ID_TRANSACTION de la tabla PMPK_TRANSACTIONS 21/10/2020 MACE
           id_transaction = GetIdTransaction (ndocumento, documentType, param_json)
          
          'Actualizar id_transaction en JSON 23/10/2020 MACE
           json_update = UpdateJsonDetailMassive(company, user, detAlmacenOld, detAlmacenNew, detAlmacenDest, ndocumento, fcreacion, fechanegocio, documentnumber, detReferenciaOld, detReferenciaNew, detCantidadOld, detCantidadNew, detPvpOld, detPvpNew, detCategoriaOld, detCategoriaNew, detTiendaOld, detTiendaNew, detStock, detPmp, detStockUpd, detStockOtherTanks, detStockOtherTanksUpd, codTypeOperation, documentType, action, total, detTipoProductoOld, detTipoProductoNew, detitem, detstockdest, detpmpdest, id_transaction)
          
          'Actualizar JSON_GENERATED de la tabla PMPK_TRANSACTIONS 23/10/2020 MACE
           UpdateJsonTransaction company, id_transaction, json_update
             
          'Llamada a la API de la COLA 21/10/2020 MACE
           PostQueuingOperations json_update
...
<form name="albaranes_pro" method="post">    
    ...
    'PMPK 16/10/2020 - MACE
        'Obtener PMPALMACEN de la tabla CONFIGURACION
        set rstAlm = Server.CreateObject("ADODB.Recordset")
        configPMPK=0
        set connPMPK = Server.CreateObject("ADODB.Connection")
        set commandPMPK =  Server.CreateObject("ADODB.Command")
        connPMPK.open session("dsn_cliente")
        commandPMPK.ActiveConnection =connPMPK
        commandPMPK.CommandTimeout = 0
        commandPMPK.CommandText="PMPK_GET_CONFIGURATION_COMPANY"
        commandPMPK.CommandType = adCmdStoredProc 'Procedimiento Almacenado
        commandPMPK.Parameters.Append commandPMPK.CreateParameter("@PARAM_IN_NCOMPANY",adVarChar,adParamInput,5,session("ncliente"))
              
		rstAlm.Open commandPMPK, , adOpenKeyset, adLockOptimistic
		   
                while not rstAlm.eof
                    configPMPK = rstAlm("PMPALMACEN")
                    rstAlm.movenext
                wend
               
        rstAlm.Close
        connPMPK.close
        set commandPMPK=nothing
        set connPMPK=nothing
    'FIN MACE
    ...
    'SRH INICIO evolutivo añadir hora
    p_hora=limpiaCadena(Request.form("hora"))
    'SRH INICIO evolutivo añadir hora
	''MPC 26/12/2007
    ...
    if p_tienda & ""="" then
		p_tienda=limpiaCadena(request.form("h_tienda"))
	end if
	...
	if viene="" then viene="SHLMX_albaranes_pro.asp"
	...
	<input type="hidden" name="caju" value="<%=caju%>"/>
	<input type="hidden" name="novei" value="<%=novei%>"/>
	<input type="hidden" name="modp" value="<%=modp%>"/>
	<input type="hidden" name="modd" value="<%=modd%>"/>
    ...
    <%if p_nalbaran & "">"" then
		if comprobar_LS(s,mode,p_nalbaran,"ALBARANES_PRO")=0 then%>
			<script language="javascript" type="text/javascript">
                ...
                document.albaranes_pro.action = "SHLMX_albaranes_pro.asp?nalbaran=&mode=add" + "&almacenSerie=<%=almacenSerie %>&almacenTPV=<%=almacenTPV %>";
                document.albaranes_pro.submit();
                parent.botones.document.location = "SHLMX_albaranes_pro_bt.asp?mode=add" + "&almacenSerie=<%=almacenSerie %>&almacenTPV=<%=almacenTPV %>";
			</script>
			<%CerrarTodo()
			response.end
		end if
   ...
   'Se crea un select para que el usuario seleccione la serie del pedido bis si fuera necesario'
    if mode="pedirserie" then
	   ...
	   CloseFila
	   DrawFila color_blau%>
	      <td class="CELDA"><input class="CELDA" type="Button" name="SeriePedidoBis" value="Aceptar" onclick="CreaPedido('<%=p_nalbaran%>');"/></td>
		  <td class="CELDA"><input class="CELDA" type="Button" name="CSeriePedidoBis" value="Cancelar" onclick="CancelaPedido('<%=p_nalbaran%>');"/></td><%
	   CloseFila%>
	   </table></p>
  	   </span>
  ...
  'Acción a realizar'
				...
				if not rstAux.eof then
					...
					if cuantos_albaranes>0 then
						...
						<%if mode="first_save" then%>
							<script language="javascript" type="text/javascript">
                                document.location = "SHLMX_albaranes_pro.asp?mode=add&almacenSerie=<%=almacenSerie %>&almacenTPV=<%=almacenTPV %>";
                                parent.botones.document.location = "SHLMX_albaranes_pro_bt.asp?mode=add"
							</script>
						<%else
							submode2=mode
							if mode="save" then
							...
							<script language="javascript" type="text/javascript">
                                document.location = "SHLMX_albaranes_pro.asp?mode=<%=mode%>&nalbaran=<%=p_nalbaran%>&almacenSerie=<%=almacenSerie %>&almacenTPV=<%=almacenTPV %>"
                                parent.botones.document.location = "SHLMX_albaranes_pro_bt.asp?mode=<%=mode%>";
							</script>
					...
					if rst.EOF then
						...
						if rst.EOF then
							...
							BorrarRegistro p_nalbaran, p_nalbaran_pro
                             %><script language="javascript" type="text/javascript">
                                   parent.botones.document.location = "SHLMX_albaranes_pro_bt.asp?mode=add";
                                   SearchPage("SHLMX_deliveryNote_pro_lsearch.asp?mode=init", 0);
		                        </script><%
						else
							rst.close
							mode="browse"%>
    ...
    if mode="edit" then
        rstAux.cursorlocation=3
		rstAux.open "select SUM(DESCUENTO + DESCUENTO2) AS DESCUENTOS_DETALLES from PROD_SHELLMX_PRE..DETALLES_ALB_PRO where NALBARAN = '" & p_nalbaran & "'", session("dsn_cliente")
    ...
    %><div class="headers-wrapper"><%
            DrawDiv "header-date","",""
                DrawLabel "","",LitFecha
				    ...
				    if mode="browse" then 
                        ...
                        DrawInput "width50","","fecha",iif(mode="add",iif(p_fecha>"",p_fecha,date()),left(rst("fecha"),10)),"onchange='validarFechaAlbaran(this)'"
                        'SRH FIN evolutivo añadir hora
                        'DrawCalendar "fecha"
						
						'Calendario modificado para mostrar sólo fecha actual
						%>
						<script>
							DrawCalendarJQuery_Today("fecha", "es");
						</script>
						<%
                    end if
            CloseDiv ' fecha
            ...
            horaUTC = rstHoraTienda("horaUTC")
            horaUTCOK = rstHoraTienda("horaUTCOK")
            'horaUTC = mid(horaUTC, 12, 8)
            'if not rstHoraTienda.eof then
            '    horaEmpresa = dateAdd("h", rstHoraTienda("OFFSETUTCHOURS"), horaUTC)
            'else
            '    horaEmpresa = horaUTC
            'end if
            DrawDiv "header-hour","",""
                DrawLabel "","",LitHora
                if mode="browse" then
                    DrawSpan "","",mid(rst("fecha"), 12, 13),""
                else
				'if mode="add" or mode="edit" then 
                    'DrawInput "width50","","hora",iif(mode="add",mid(horaUTC, 12, 8),mid(rst("fecha"), 12, 8)),""
                    DrawInput "width50","","hora",iif(mode="add",horaUTCOK,mid(rst("fecha"), 12, 8)),""
                end if
            CloseDiv ' hora           
            ...
            DrawDiv "header-nproveedor","",""
                ...
                Formulario="SHLMX_albaranes_pro"
			    if mode="browse" then
				    if rst("nproveedor")>"" then%>
					    <%=Hiperv_SHLMX(OBJProveedores,rst("nproveedor"),"","",Permisos,Enlaces,session("usuario"),session("ncliente"),trimCodEmpresa(rst("nproveedor")),LitVerProveedor)%>
				    <%end if
			    else
				    %><input class="CELDA width20" type="text" name="nproveedor" value="<%=TrimCodEmpresa(iif(tmp_nproveedor>"",tmp_nproveedor,rst("nproveedor")))%>" size="8" onchange="TraerProveedor('<%=mode%>');"/> <!-- onchange="TraerSerie('<%=mode%>');"-->
				    <a class="CELDAREFB"  href="javascript:AbrirVentana('SHLMX_proveedores_busqueda.asp?ndoc=albaranes_pro&titulo=<%=LitSelProv%>&mode=search','P',<%=AltoVentana%>,<%=AnchoVentana%>)" onmouseover="self.status='<%=LitVerProveedor%>'; return true;" onmouseout="self.status=''; return true;"><img src="<%=themeIlion%><%=ImgBuscar%>" <%=ParamImgBuscar%> alt="<%=LitBuscarProveedor%>" title="<%=LitBuscarProveedor%>"/></a><%
			    end if
	    ...
	    if not rst.eof then
		    if mode="browse" and (rst("nfactura")&"")="" then
		        ...
		        <span class='header-note-buttonNote'>
			        <img src="<%=themeIlion%><%=ImgAnotar%>" style="cursor:pointer;" <%=ParamImgAnotar%> alt="<%=LITANOTARCAJA%>" title="<%=LITANOTARCAJA%>" onclick="Acaja('<%=rst("nalbaran")%>')"/>
  		        </span><%
                rstAux.cursorlocation=3
	...
	if mode="browse" or mode="save" then
        ...
        ''''''''
	    seleccion = "select 'SHLMX_albaranes_pro_imp.asp' as fichero, a.descripcion as descripcion,a.personalizacion,b.parametros as parametros from clientes_formatos_imp as a with(nolock), formatos_imp as b with(nolock) where a.nformato=b.nformato and a.ncliente='"&session("ncliente")&"' and b.tippdoc='ALBARAN DE PROVEEDOR' order by descripcion"
	    ...
	    if si_tiene_modulo_21 = 0 and si_tiene_modulo_22 = 0 then
            ...
            %>
                <a class="CELDAREFB" href="javascript:AbrirVentana('../../../central.asp?pag1=./Servicios/recursos.asp&pag2=./Servicios/recursos_bt.asp&codigo=<%=nalbaran%>&codigo_print=<%=p_nalbaran_pro%>&tipo=albaran de proveedor&viene=enlaces', 'P', <%=AltoVentana%>, <%=AnchoVentana%>)">&nbsp;&nbsp;&nbsp;<%=LitEnlaces%>&nbsp;&nbsp;&nbsp;</a>
            <%
            CloseDiv
        end if
        DrawDiv "header-print","",""
	    %><label><a id="idPrintFormat" class="CELDAREFB" href="javascript:AbrirVentana(document.albaranes_pro.formato_impresion.value+'nalbaran=<%="(\'"+p_nalbaran+"\')"%>&mode=browse&empresa=<%=session("ncliente")%>&novei=<%=novei%>','I',<%=AltoVentana%>,<%=AnchoVentana%>)" onmouseover="self.status='<%=LitImpresionConFormato%>'; return true;" onmouseout="self.status=''; return true;"><%=LitImpresionConFormato%></a></label><%
	...
	if (mode="browse" or mode="edit" or mode="add") and not rst.EOF then%>
		...
		<input type="hidden" name="h_nalbaran_pro" value="<%=rst("nalbaran_pro")%>"/>
		<input type="hidden" name="h_nbalance" value="<%=rst("nbalance")&""%>"/>			
		<input type="hidden" id="h_tienda" name="h_tienda" value="<%=rst("tienda")&""%>"/><%			
                ...
                <%
                    'DrawFila color_blau
                        if mode="browse" then
                            ...
                            EligeCeldaResponsive "text", mode, "CELDA", "", "", 0, LitSerie, "serie", "",trimCodEmpresa(rst("serie")) & " - " & rst("nomserie")
                        ...
                        if mode = "browse" then
                            ...
                            EligeCeldaResponsive "check", mode, "CELDA", "", "", 0, LitValorado, "valorado", "",iif(p_valorado>"",nz_b(p_valorado),rst("valorado"))
                        end if
	...
	''response.write("los datos 3 son-" & mode & "-" & datoDivisa & "-" & tmp_divisa & "-" & rst("divisa") & "-<br>")
				        ...
				        if si_tiene_modulo_proyectos<>0 then
					        ...
					        if mode <> "browse" then
                                %><div class="col-lg-4 col-md-6 col-sm-6 col-xs-12"><%
						            ...
						            %><iframe id='frProyecto' name='fr_Proyecto' src='../../../mantenimiento/docproyectos.asp?viene=albaranes_pro&mode=<%=mode%>&cod_proyecto=<%=iif(tmp_cod_proyecto>"",tmp_cod_proyecto,iif(isnull(rst("cod_proyecto")),"",rst("cod_proyecto")))%>' width='250' height='30' frameborder="no" scrolling="no" noresize="noresize"></iframe><%
					            %></div><%
                            else
			        ...
			        'DrawFila color_blau
				        ...
				        if mode <> "browse" then
                            'DVV: solo usuario admin puede editar, los otros usuarios solo visualizarán el centro de costo
                            if mode = "edit" and session("user_admin")&"" = "0" then
                                %><div class="col-lg-4 col-md-6 col-sm-6 col-xs-12"><%
                                    DrawLabel "","",LitTienda
                                    DrawSpan "","",d_lookup("descripcion","tiendas","codigo='" & rst("tienda") & "'",session("dsn_cliente")),""
                                %></div><%
                            else
                                ...
                                rstAux.Open "EXEC MXPRY_GetStoresByEmployed @PI_NCOMPANY = '" & session("ncliente") & "', @PI_vUserCode = '" & session("ncliente") + session("usuario") & "'", dsn_shlmxproy 'Cargado de tiendas por usuario - DVV
	    			            DrawSelectCelda "CELDALEFT","","",0,LitTienda,"tienda",rstAux,defecto,"codigo","descripcion","onchange","javascript:actualizarHTienda(this);"
		    		            rstAux.close
                            end if
			            else
		            ...
		            'DrawFila color_blau
			            ...
			            if mode="browse" then
                            ...
                            %><div class="col-lg-4 col-md-6 col-sm-6 col-xs-12"><%
						        ...
						        %><input class="width60" maxlength="25" type="text" name="ncuenta_pro" value="<%=num_cuenta%>"/><%
                            %></div><%
                        end if
		            'CloseFila
		            ...
		            'DrawFila color_blau
		                ...
		                if mode="browse" then		        
                            ...
                            %><div class="col-lg-4 col-md-6 col-sm-6 col-xs-12"><%
						        ...
						        %><input class="width60" disabled type="text" name="banco" value="<%=banco%>"/><%
                            %></div><%
			            end if
		            'CloseFila
			        'DrawFila color_blau
				        ...
				        if mode="browse" then
                            ...
                            %><div class="col-lg-4 col-md-6 col-sm-6 col-xs-12"><%
						        ...
						        %><input class="width60" maxlength="50" type="text" size="25" name="fob" value="<%=defecto%>"/><%
					        %></div><%
				        end if
			       ' CloseFila
			        if mode="browse" then
					        ...
					        while not rstAux2.eof
						        ...
						        if rstAux2("titulo") & "">"" then
							        ...
							        elseif rstAux2("tipo")=4 then
                                        ...
                                        DrawDiv "1","",""
                                        DrawLabel "","",rstAux2("titulo") & ":"%><input type="text" style="width:155px;" class="CELDA" name="<%="campo" & num_campo%>" value="<%=valor_campo_perso%>"/>
								        <%CloseDiv
					        ...
					        while not rstAux2.eof
						        ...
						        if rstAux2("titulo") & "">"" then
							        ...
							        if rstAux2("tipo")=1 then
                                        ...
                                        DrawDiv "1","",""
                                        DrawLabel "","",rstAux2("titulo") & ":"%><input type="text" class="CELDA" name="<%="campo" & num_campo%>" style='width:200px' value="<%=valor_campo_perso%>" />
								        <%CloseDiv
							        elseif rstAux2("tipo")=2 then
								        if valor_campo_perso="" then
									        valor_campo_perso=0
		...
		if mode="browse" then
			...
			if rst("dir_envio")>"" then
				pagina="../../../central.asp?pag1=./custom/ShellMexico/compras/SHLMX_albaranes_prodireccion_env.asp&ndoc=" &rst("nalbaran") & "&mode=browse&pag2=./custom/ShellMexico/compras/SHLMX_albaranes_prodireccion_env_bt.asp&titulo=" & ucase(LITDATOSENVIO) & " " & rst("nalbaran_pro")
			else
				pagina="../../../central.asp?pag1=./custom/ShellMexico/compras/SHLMX_albaranes_prodireccion_env.asp&ndoc=" &rst("nalbaran") & "&mode=edit&pag2=./custom/ShellMexico/compras/SHLMX_albaranes_prodireccion_env_bt.asp&titulo=" & ucase(LITDATOSENVIO) & " " & rst("nalbaran_pro")
			end if
			...
			if not rstAux.eof then
				if not isnull(rstAux("nfactura_pro")) then
					%><input type="hidden" name="h_nfactura" value="<%=rstAux("nfactura_pro")%>"/><%
				else
                    ...
                    <div id="tabs" style="display:none">
                            ...
                            <div id="tabs-details" class="overflowXauto">
		   	                    <table class="width90 md-table-responsive bCollapse" >
		   	                        <%DrawFila color_terra
					                    ...
					                    <td class='CELDAL7 underOrange width10'>ID Producto Proveedor</td>
                                        <td class='CELDAR7 underOrange width15'">Nombre</td>
					                    ...
					                    <td class='CELDAR7 underOrange width5'>Costo</td>
					                    <td class='CELDAR7 underOrange width5'>IEPS %</td>
					                    <td class='CELDAR7 underOrange width5'>IEPS $</td>
					                    <td class='CELDAR7 underOrange width5'>Tipo Dto</td>
					                    ...
					                    <td class='CELDAR7 underOrange width5'><%=LitIva%> $ </td>
					                    <td class='CELDAR7 underOrange width5'>Subtotal</td>
				                ...
				                <%if isnull(rst("nfactura")) and isnull(rst("nbalance")) then
					                ...
					                <iframe id='frDetallesIns' name='fr_DetallesIns' class="width90 iframe-input md-table-responsive" src='SHLMX_albaranes_prodetins.asp?ndoc=<%=rst("nalbaran")%>&nproveedor=<%=rst("nproveedor")%>&tienda=<%=rst("tienda")%>&modp=<%=modp%>&modd=<%=modd%>&modi=<%=modi%>&almacenSerie=<%=almacenSerie %>&almacenTPV=<%=almacenTPV %>' frameborder="no" scrolling="no" noresize="noresize"></iframe>
					                <%else%>
					                <iframe id='frDetallesIns' name='fr_DetallesIns' class="width90 iframe-input md-table-responsive" src='SHLMX_albaranes_prodetins.asp?ndoc=<%=rst("nalbaran")%>&nproveedor=<%=rst("nproveedor")%>&tienda=<%=rst("tienda")%>&modp=<%=modp%>&modd=<%=modd%>&modi=<%=modi%>&almacenSerie=<%=almacenSerie %>&almacenTPV=<%=almacenTPV %>' frameborder="no" scrolling="no" noresize="noresize"></iframe>
					                <%end if%>
				                <%end if%>
				                <iframe id='frDetalles' name="fr_Detalles" class="width90 md-table-responsive" src='SHLMX_albaranes_prodet.asp?ndoc=<%=rst("nalbaran")%>&nproveedor=<%=rst("nproveedor")%>&tienda=<%=trimCodEmpresa(rst("tienda"))%>&modp=<%=modp%>&modd=<%=modd%>&modi=<%=modi%>&almacenSerie=<%=almacenSerie %>&almacenTPV=<%=almacenTPV %>' height='150' frameborder="yes" noresize="noresize"></iframe>
                        ...
                        <div id="tabs-concepts" class="overflowXauto" >
				            ...
				            DrawFila color_terra%>
					            ...
					            <td class='CELDAR7 underOrange width5'><%=LitPVP%> + IEPS</td>
					            <td class='CELDAR7 underOrange width5'><%=LitDto%></td>
					            <td class='CELDAR7 underOrange width5'><%=LitIva%></td>
				            ...
				            if isnull(rst("nfactura")) and isnull(rst("nbalance")) then
					            ...
					            DrawFila color_blau
						            ...
						            <td class=" underOrange width10 ">
							            <a class="ic-accept noMTop" href="javascript:addConcepto('<%=nalbaran%>');" onblur="javascript:document.albaranes_pro.cantidad.focus();"><img src="<%=themeIlion%><%=ImgNuevo%>" <%=ParamImgNuevo%> alt="<%=LitNuevo%>" title="<%=LitNuevo%>"/></a>
						            </td><%
						            if oculta=1 then%>
						              <script language="javascript" type="text/javascript">
                                          document.albaranes_pro.cantidad.focus();
				            ...
				            %></table>
				            <iframe id="frConceptos" name="fr_Conceptos" class="width90 md-table-responsive" src='SHLMX_albaranes_procon.asp?mode=browse&ndoc=<%=rst("nalbaran")%>' frameborder="yes" noresize="noresize"></iframe>
			            </div>
                        <%if vencesp="1" then%>
                            <div id="tabs-vencimientos">
			                    <table style="border-collapse:collapse;table-layout:fixed;width:355px" cellpadding="1" cellspacing="1" ><%
						                    ...
						                    DrawFila color_blau
							                    %>
							                        ...
							                        <td class='CELDAL7' style="width:25px;">
								                        <a class="ic-accept noMTop" href="javascript:addVencimiento('<%=rst("nalbaran")%>');" onblur="javascript:document.albaranes_pro.fechaVto.focus();"><img src="<%=themeIlion%><%=ImgNuevo%>" <%=ParamImgNuevo%> alt="<%=LitNuevo%>" title="<%=LitNuevo%>"/></a>
							                        </td>
                                                <%
						                    CloseFila
                                        end if
				                    ...
				                    %><iframe id="frVencimientos" name="fr_Vencimientos" class="width90 md-table-responsive" src='../../../compras/vencimientos_pro_config.asp?mode=browse&tdocumento=albaranes_pro&ndoc=<%=rst("nalbaran")%>' width='410' height='200' frameborder="yes" noresize="noresize"></iframe>
                            </div>
                        <%end if%>
                        <div id="tabs-payments" class="overflowXauto">
				            ...
				            %></table>
				            <iframe id="frPagosCuenta" name="fr_PagosCuenta" class="width90 md-table-responsive" src='SHLMX_albaranes_propagos.asp?mode=browse&ndoc=<%=rst("nalbaran")%>' width='650' height='80' frameborder="yes" noresize="noresize"></iframe>
			            </div>
  				                        ...
  				                        <table BORDER="1" cellspacing="0" cellpadding="0">
                                            ...
                                            <tr>
  					                            <td CLASS=ENCABEZADOC height="25">	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<%=LitDatosEnvio%>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
  				                                    <%if isnull(rst("nfactura")) then%>
  						                                <a class='CELDAREFB'  href="javascript:AbrirVentana('<%=pagina%>','P',<%=altoventana%>,<%=anchoventana%>)" OnMouseOver="self.status='<%=LitEditar%>'; return true;" OnMouseOut="self.status=''; return true;">
  						                                <%=LitEditar%></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
  					                                <%end if%>
  					                            </td>
                                            </tr>
        ...
        <div class="Section" id="S_TOTAL">
            ...
            <div class="SectionPanel" id="DATTOTAL" >
                    ...
                    CloseDiv%>
                    <%
                        xxxcantidad	= null_z(d_sum("cantidad","detalles_alb_pro","nalbaran='" & rst("nalbaran") & "'",session("dsn_cliente")))
                        xxxcantidadcon	= null_z(d_sum("cantidad","conceptos_alb_pro","nalbaran='" & rst("nalbaran") & "'",session("dsn_cliente")))
                    ...
                    <%DrawDiv "4", "", ""
			            ...
			            <%DrawDiv "4", "", ""
                            DrawLabel "", "",LitDto
                            if mode<>"browse" then
					            %><input class="ENCABEZADOR" type="text" name="dto1" value="<%=iif(tmp_dto1>0,tmp_dto1,iif(rst("descuento")>"",rst("descuento"),0))%>" onchange="RoundNumValue(this,<%=decpor%>);Precios();" <%=iif(countTotalDetail > 0,"disabled","")%> /><%
				            else
			            ...
			            <%DrawDiv "4", "", ""
                            DrawLabel "", "",LitDto2
                            if mode<>"browse" then
					            %><input class=ENCABEZADOR type="text" name="dto2" value="<%=iif(tmp_dto2>0,tmp_dto2,iif(rst("descuento2")>"",rst("descuento2"),0))%>" onchange="RoundNumValue(this,<%=decpor%>);Precios();" <%=iif(countTotalDetail > 0,"disabled","")%>/><%
				            else
                        ...
                        <%if ((rst("recargo")<>0) or mode="edit" or mode="add") then%>
					        <%DrawDiv "4", "", ""
                                DrawLabel "","",LitRF
                                if mode<>"browse" then
						            %><input CLASS=ENCABEZADOR type="text" name="rf" value="<%=iif(tmp_rf>0,tmp_rf,iif(rst("recargo")>"",rst("recargo"),0))%>" onchange="RoundNumValue(this,<%=decpor%>);Precios();"/><%
					            else
				        ...
				        if ((rst("irpf")<>0) or mode="edit" or mode="add") then%>
					        <%DrawDiv "4", "", ""
                                DrawLabel "","",Litirpf
                                    if mode<>"browse" then
						                %><input CLASS=ENCABEZADOR type="text" name="irpf" value="<%=iif(tmp_irpf>0,tmp_irpf,iif(rst("irpf")>"",rst("irpf"),0))%>" onchange="RoundNumValue(this,<%=decpor%>);Precios();"/><%
					                else
                        ...
                        DrawDiv "4", "", ""
				            ...
				            %><input class="CELDA" type="hidden" name="IRPF_Total" value="<%=iif(tmp_IRPF_Total>"",tmp_IRPF_Total,iif(isnull(rst("IRPF_Total")),"",rst("IRPF_Total")))%>"/><%

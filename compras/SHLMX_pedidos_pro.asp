...
''ricardo 5-6-2003 se a�ade el parametro novei para que en los formatos de impresion no salga el item
''''ricardo 31/7/2003 comprobamos que existe el pedido que se ha pedido ver desde un listado, sino se va al modo add
'***RGU 19/12/2005 : A�adir limite de compras para el mes en que vence el pago del pedido
'                    Falta gestionar el incremento, descuento de este limite cuando se introducen borran/detalles
'					 Falta gestionar cuando se supera el limite, si seguir mediante contrase�a o impedir mas detalles
' jcg 20/01/2009: A�adida la columna proyecto al proveedor y tratamiento de la misma.
%>
...
<!--#include file="../shlmx_conexion.inc" -->
...
<!--#include file="../../../compras/pedidos_pro.inc" -->
...
<!--#include file="SHLMX_pedidos_proActionDrop.inc" -->
<!--#include file="SHLMX_pedidos_pro_linkextra.inc" -->
...
'btorreme - Se añade logica para obtener tiendas del usuario
set rsAux = Server.CreateObject("ADODB.Recordset")
 
if rsAux.State<>0 then rsAux.Close
cadena= "exec MXPRY_VERIFY_USER_IS_ADMIN '" & session("ncliente") & "','" & session("usuario") & "'"
rsAux.Open cadena,dsn_shlmxproy,adOpenKeyset,adLockOptimistic
 
if rsAux.eof then
	session("user_admin") = ""
else
	session("user_admin") = rsAux("RESULT")
end if
rsAux.close
 
set rsAux=NOTHING
 
' Mostrar el valor de los permisos en la consola del navegador
Response.Write "<script>console.log('Usuarion admin: " & session("user_admin") & "');</script>"
'btorreme - Se añade logica para obtener tiendas del usuario
...
<script language="javascript" type="text/javascript">
    ...
    function ComprobarPvp(modo, npedido, nempresa, usuario, novei, pagina, nproveedor) {
        if (document.pedidos_pro.h_nalbaran.value == "NO" && document.pedidos_pro.h_nfactura.value == "NO") {
            if (document.pedidos_pro.rpc.value == nproveedor) {
                ...
                document.pedidos_pro.action = "SHLMX_pedidos_pro.asp?mode=browse&submode=crearPedCentral&npedido=" + npedido;
    ...
    function TraerProveedor(mode, modo) {
        ...
        cambiar_cliente = 0; //btorreme - Se seteo por defecto 0 para evitar recarga

        //btorreme - Se comento validación de serie
        ...
        if (mode == "add") {
            ...
            //btorreme - Se envía a la variable "&nproveedor=" la variable prov_old anteriormente enviado vació 
            document.location.href = "SHLMX_pedidos_pro.asp?ndoc=" + document.pedidos_pro.h_npedido.value
                + "&nproveedor=" + prov_old
                + "&mode=" + mode
    ...
    function Recalcula(total_iva_bruto, total_re_bruto) {
        ...
        document.pedidos_pro.base_imponible.value = parseFloat(document.pedidos_pro.base_imponible.value).toFixed(<%=NdecDiPedido %>);
        ...
        document.pedidos_pro.total_descuento.value = parseFloat(document.pedidos_pro.total_descuento.value).toFixed(<%=NdecDiPedido %>);
        ...
        document.pedidos_pro.total_iva.value = parseFloat(document.pedidos_pro.total_iva.value).toFixed(<%=NdecDiPedido %>);
        ...
        document.pedidos_pro.total_re.value = parseFloat(document.pedidos_pro.total_re.value).toFixed(<%=NdecDiPedido %>);
        ...
        document.pedidos_pro.total_recargo.value = parseFloat(document.pedidos_pro.total_recargo.value).toFixed(<%=NdecDiPedido %>);
        ...
        document.pedidos_pro.total_irpf.value = parseFloat(document.pedidos_pro.total_irpf.value).toFixed(<%=NdecDiPedido %>);
        ...
        document.pedidos_pro.total_pedido.value = parseFloat(document.pedidos_pro.total_pedido.value).toFixed(<%=NdecDiPedido %>);
    }
    ...
    function addConcepto(npedido) {
        ...
        //vamos a comprobar si hay l�mites de compra y si los hay si se cumplen
        alcanzado = document.pedidos_pro.alcanzado.value;    
        if (alcanzado != "a")  //el l�mite existe
        {
    ...
    //Calcula el importe de la l�nea de detalle del concepto.
    function ImporteDetalle() {
        console("Ingresó a ImporteDetalle");
        ...
        if (isNaN(document.pedidos_pro.cantidad.value.replace(",", ".")) || isNaN(document.pedidos_pro.descuento.value.replace(",", ".")) || isNaN(document.pedidos_pro.pvp.value.replace(",", ".")))
            ...
            pvpSinComas = document.pedidos_prodetins.h_costpr.value.replace(",", ".");//MACE - Coste sin impuestos por Proveedor
            iepsuno = document.pedidos_prodetins.h_iepsmon.value.replace(",", ".");//MACE - IEPS $ por Referencia

            cantidadSinComas = document.pedidos_pro.cantidad.value.replace(",", ".");
            dtoSinComas = document.pedidos_prodet.descuento.value.replace(",", ".");
            dto2SinComas = document.pedidos_prodet.descuento2.value.replace(",", ".");

            pvpcant = parseFloat(pvpSinComas)*parseFloat(cantidadSinComas);//MACE - COSTO PROV + IEPS $
            pvpieps=parseFloat(pvpcant)+parseFloat(iepsuno);

		    pelas_descuento=pvpieps-(pvpieps*parseFloat(dtoSinComas))/100;
            pelas_descuento2=pelas_descuento-(pelas_descuento*parseFloat(dto2SinComas))/100;
		    importe = parseFloat(pelas_descuento2).toFixed(<%=NdecDiPedido %>);
    ...
    function pedir(npedido) {
        if (window.confirm("<%=LitConfPedPro%>"))
            if (document.getElementById("waitBoxOculto").style.visibility == "hidden") {
                ...
                document.getElementById("waitBoxOculto").style.visibility = "visible";
                document.pedidos_pro.action = "SHLMX_pedidos_pro.asp?mode=browse&submode=pedir_pro&npedido=" + npedido;
                document.pedidos_pro.submit();
            }
    }
    ...
    function MasDet(sentido, lote, firstReg, lastReg, campo, criterio, texto, firstRegAll, lastRegAll) {
        fr_Detalles.document.pedidos_prodet.action = "SHLMX_pedidos_prodet.asp?mode=browse&sentido=" + sentido + "&lote=" + lote + "&campo=" + campo + "&criterio=" + criterio + "&texto=" + texto + "&firstReg=" + firstReg + "&lastReg=" + lastReg + "&firstRegAll=" + firstRegAll + "&lastRegAll=" + lastRegAll + "&almacenSerie=<%=almacenSerie %>&almacenTPV=<%=almacenTPV %>";
    ...
    //*** i AMP Nueva funci�n cambiar divisa con factor de cambio incorporado.
    var ret_tra = "";
    var ret_tra2 = "";
    ...
    //btorreme - Guardamos el value de tienda en el input hidden para validar campo requerido - 
    // function actualizarHTienda(combo) {
    //     var hTiendaInput = document.getElementById("h_tienda");
    //     hTiendaInput.value = combo.value;
    // }
    //btorreme
...
<%function CalcularNumDocumentoDSN(nserie,fecha,dsnCentral)
	...
	'Actualizar el n� de cliente de CONFIGURACION.
...
function CabeceraPedido(npedido,nserie,fecha,ncliente,nempresaCentral,dsnCentral)
	if npedido="" then
		...
		'Gestionamos las direcciones de env�o que marca en el pedido de franquicia
		...
		if not rstCli.EOF then
            ...
            if domicilioEnvio&"">"" then
                'Asignamos la direcci�n ya existente
                rst("dir_envio")=domicilioEnvio
            else
                'Creamos nueva direcci�n en central y asignamos al pedido de venta
	...
	'Obtener el siguiente n� de documento de la tabla series.
	if npedido="" then
...
'Actualiza los datos del registro cuando se pulsa el bot�n de 'GUARDAR'
sub GuardarRegistro(npedido,nserie)
	...
	if npedido="" then
		...
		if Dom="FALSE" then
			...
			<script language="javascript" type="text/javascript">
                ...
                document.location = "SHLMX_pedidos_pro.asp?mode=add"
                parent.botones.document.location = "SHLMX_pedidos_pro_bt.asp?mode=add"
			</script>
		<%end if
	end if
	FechaDoc=rst("fecha")
    ...
    'btorreme 02/04/2024
    horaForm = Nulear(Request.Form("hora"))
    fechaForm = Nulear(Request.Form("fecha"))
    fechaHora = fechaForm & " " & horaForm    
	rst("fecha")		= fechaHora
    'btorreme 02/04/2024
	...
	rst("fob")=nulear(enc.EncodeForJavascript(request.form("fob")))

    'btorreme 05/02/2024
    if request.form("mode") = "edit" then
        if session("user_admin")&"" <> "0" then
            rst("campo01")=nulear(request.form("tienda"))
        end if
    else
        rst("campo01")=nulear(request.form("tienda"))
    end if
	...
	rst("notas")= Nulear(enc.EncodeForJavascript(request.form("notas")))
        ...
        if cambio_proveedor=true then
            ...
            while not rstAux.eof
                ...
                if not rstMiProveer.eof then
                    ieps_imp	= null_z(d_sum("N_IEPS","PRY_IEPS_ARTICULOS","V_REFERENCE='" & rstaux("referencia") & "'",dsn_shlmxproy))'MACE
                    'importe_imp	= null_z(d_sum("importe","detalles_ped_pro","npedido='" & npedido & "'",session("dsn_cliente")))'MACE
                    
					TmpPvp=CambioDivisa(rstMiProveer("importe"), rstMiProveer("divisa"),request.form("h_divisa"))
					impOrig=TmpPvp
                    
					TmpDto=null_z(rstaux("descuento")) 'MACE
					TmpDto2=null_z(rstaux("descuento2")) 'MACE
                    
                    ieps_imp = ieps_imp*rstAux("cantidad")
					TmpPVP=TmpPVP*rstAux("cantidad")

                    TmpPVP=TmpPVP+miround(ieps_imp,DEC_PREC)
                    
					TmpPVP	= (TmpPVP)*(100-null_z(TmpDto))/100
					TmpImporte	= TmpPVP*(100-null_z(TmpDto2))/100
					TmpImporte=miround(TmpImporte,2)
    ...
    ''ricardo 28/4/2003 si el usuario ha querido recalcular los importes al cambiar las propiedades de la cabecera
    if request.querystring("recalcular_importes")="1" then
        'Detectar cambios en la divisa del documento para cambiar la divisa de los detalles(art�culos y conceptos)
	...
	'rst("campo01")=lista_valores(1) 'BTORREME
	...
	while not rstAux.EOF
        'ACTUALIZAR TOTALES - MACE
        total_iva_bruto	= null_z(d_sum("((pvp*iva)/100)*cantidad","detalles_ped_pro","npedido='" & rst("npedido") & "'",session("dsn_cliente")))
        total_imp_bruto	= null_z(d_sum("importe","detalles_ped_pro","npedido='" & rst("npedido") & "'",session("dsn_cliente")))
        total_imp_total = total_imp_bruto + total_iva_bruto
        'total_imp_total = miround(null_z(total_imp_total),4)
        'FIN
		...
		SumImporteBruto=SumImporteBruto + total_imp_bruto
		dto1=miround((null_z(total_imp_bruto)*null_z(rst("descuento")))/100,4)
		dto2=miround(((null_z(total_imp_bruto)-dto1)*null_z(rst("descuento2")))/100,4)
		total_descuento=dto1+dto2+dto3
		SumTotalDto=SumTotalDto + null_z(total_descuento)
		'base_imponible=null_z(rstAux("suma"))-null_z(total_descuento)
		base_imponible=null_z(total_imp_bruto)
		SumBaseImponible=SumBaseImponible + null_z(base_imponible)
	...
	rst("total_iva")=miround(total_iva_bruto,n_decimales)
	...
	rst("total_pedido")=miround(null_z((total_imp_bruto+total_iva_bruto)-rst("total_descuento")),2) 'MACE
	rst.Update
...
function BorrarRegistro(npedido)
	...
	'Se gener� factura o albar�n. Imposible eliminar
	if rst("nfactura")>"" or rst("nalbaran")>"" or tiene_caja=1 then
		if rst("nfactura")>"" then%>
			<script language="javascript" type="text/javascript">
                ...
                %>
                window.alert("<%=LitMsgBorrarPedido & " " & alertMessage%>");
			</script>
		...
		elseif rst("nalbaran")>"" then%>
			<script language="javascript" type="text/javascript">
                ...
                %>
                window.alert("<%=LitMsgBorrarPedido2 & " " & alertMessage%>");
            </script>
			<%BorrarRegistro=false
	...
	'Antes de borrar se modifica el stock pendiente de recibir
		...
		'Miramos si se va a borrar el �ltimo generado y si es as� se descuenta el contador de documentos
        ...
        'Borrado y Actualizaci�n de stocks
		while not rstAux.EOF
...
sub GuardaDetalle(npedido,nserie)
	'Obtener el �ltimo n� de detalle del pedido.
...
<form name="pedidos_pro" method="post">
    <%
    PintarCabecera "SHLMX_pedidos_pro.asp"
    ...
    if formato_impresionEleg & ""="" then
        formato_impresionEleg=Request.form("formato_impresionEleg")
    end if
    ...
    ' >>> MCA 11/04/05 : A�adir par�metro de usuario con la(s) serie(s) a los documentos de compra.
    '					 pedidos_pro.asp  bloque 1/4
    s=limpiaCadena(request.querystring("s"))
    if s="" then s=limpiaCadena(request.form("s"))
        s=preparar_lista(s)

        'iframe oculto que utilizaremos para comprobar los l�mites de compras, s�lo nos hace falta en mode edit%>
        <iframe id='comprobar_limites' src='SHLMX_comprobar_limites.asp?viene=pedpro' style="display:none;" width='0' height='0' frameborder="no" scrolling="no" noresize="noresize"></iframe>
        ...
        if request.querystring("prov")>"" then
            nproveedor=limpiaCadena(request.querystring("prov"))
        else
            nproveedor=limpiaCadena(request.form("prov"))
        end if
        ...
        if tmp_serie & ""="" then
            tmp_serie=limpiaCadena(Request.form("serie"))
        end if
        observacionesR=limpiaCadena(request.querystring("observaciones"))
        notasR=limpiaCadena(request.querystring("notas"))
        serieR=limpiaCadena(Request.Form("serie"))
        fechaR=limpiaCadena(Request.Form("fecha"))
        npedidoH=limpiaCadena(Request.Form("h_npedido"))
        ...
        if p_npedido & "">"" then
            if comprobar_LS(s,mode,p_npedido,"PEDIDOS_PRO")=0 then%>
                <script language="javascript" type="text/javascript">
                    ...
                    document.pedidos_pro.action = "SHLMX_pedidos_pro.asp?npedido=&mode=add";
                    document.pedidos_pro.submit();
                    parent.botones.document.location = "SHLMX_pedidos_pro_bt.asp?mode=add";
                </script>
                <%CerrarTodo()
        ...
        ' <<< MCA 11/04/05 : A�adir par�metro de usuario con la(s) serie(s) a los documentos de compra.
                    ...
                    if serieCliente&""<="" then
                        ...
                        if serieCliente&""<="" then
                            ...
                            <script language="javascript" type="text/javascript">
                                ...
                                document.location = "SHLMX_pedidos_pro.asp?npedido=<%=doc %>&mode=browse"
                            </script><%
                        end if
                    end if
            ...
            'JMA 16/12/04 si existen campos personalizables con titulo no nulo saldr�n los campos personalizables'
            ...
            <%'JMA 16/12/04 FIN si existen campos personalizables con titulo no nulo saldr�n los campos personalizables'
            ...
            'JMA 16/12/04 a�adir campos personalizables a pedidos_pro'
            ...
            'Captura de datos del articulo que se est� introduciendo en el detalle
            if nproveedor > "" then
                if len(nproveedor)<=5 then
                    nproveedor=session("ncliente") & Completar(nproveedor,5,"0")
                end if
            ...
            elseif mode="delete" then
                ...
                if not rst.eof then
                    ...
                    if BorrarRegistro(npedidoH)=true then
                        ...
                        %><script language="javascript" type="text/javascript">
                            parent.botones.document.location = "SHLMX_pedidos_pro_bt.asp?mode=add";
                            SearchPage("custom/ShellMexico/compras/SHLMX_purchaseOrder_lsearch.asp?mode=init", 0);
                        </script><%
                    else
                        mode="browse"
                    end if
            ...
            'Mostrar los datos de la p�gina.

            'btorreme - Se agrega consulta para bloqueo de inputs según detalle del albaran
            if mode="edit" then
                rstAux.cursorlocation=3
                rstAux.open "select SUM(DESCUENTO + DESCUENTO2) as DESCUENTOS_DETALLES from PROD_SHELLMX_PRE..DETALLES_PED_PRO where NPEDIDO = '" & npedido & "'", session("dsn_cliente")
                if not rstAux.eof then countTotalDetail=rstAux("DESCUENTOS_DETALLES")
                rstAux.close
                Response.Write "<script>console.log('countTotalDetail: " & countTotalDetail & "');</script>"
            end if
            'btorreme
            ...
            if mode="browse" or mode="edit" then
                ...
                commandAux.ActiveConnection =connAux
                commandAux.CommandTimeout = 60
                commandAux.CommandText= "select f.*, CONVERT(varchar, f.fecha, 108) AS hora_formateada,d.abreviatura,d.ndecimales,c.razon_social from pedidos_pro as f,proveedores as c,divisas as d where c.nproveedor=f.nproveedor and npedido=? and f.divisa=d.codigo"
                commandAux.CommandType = adCmdText
                commandAux.Parameters.Append commandAux.CreateParameter("@npedido", adVarChar,adParamInput,20, npedido)
            ...
            elseif mode="add" then
                ...
                commando.CommandText= "select *, CONVERT(varchar, fecha, 108) AS hora_formateada,'' as razon_social from pedidos_pro where npedido=?"
            ...
            if mode="edit" then
                ...
                %>
                <input type="hidden" name="h_formapago" value="<%=rst("forma_pago")%>"/><%
                ''FALLA EncodeForHtmlAttribute(rst("forma_pago"))
            end if
                
            %><div class="headers-wrapper"><%
                'btorreme 02/04/2024
                set rstHoraTienda = Server.CreateObject("ADODB.Recordset")
                rstHoraTienda.CursorLocation=3
                seleccionHoraTienda = "select ct.OFFSETUTCHOURS, DATEADD(HOUR, ISNULL(ct.OFFSETUTCHOURS, 0), GETUTCDATE()) as horaUTC ,  CONVERT(varchar, DATEADD(HOUR, ISNULL(ct.OFFSETUTCHOURS, 0), GETUTCDATE()), 8 ) as horaUTCOK from clientes c with(nolock) inner join CONFIG_TIMEZONES ct on c.TIMEZONEID = ct.ID  where ncliente='"&session("ncliente")&"'"
                rstHoraTienda.Open seleccionHoraTienda, DsnIlion
                horaUTCOK = rstHoraTienda("horaUTCOK")

                DrawDiv "header-date","",""
                    DrawLabel "txtMandatory","",LitFecha
                    if mode="browse" then
                        'DrawSpan "","",rst("fecha"),""
                        DrawSpan "","",left(rst("fecha"), 10),""
                    else
                        DrawInput "width50","","fecha",iif(mode="add",iif(tmp_fecha>"",tmp_fecha,date()),left(rst("fecha"), 10)),"onchange='javasript:CalculaFechaPago();'"
                        DrawCalendar "fecha"
                        DrawInput "'width20 hidden'","","hora",iif(mode="add",horaUTCOK, rst("hora_formateada")),""
                    end if
                CloseDiv
                set rstHoraTienda = nothing
                'btorreme 02/04/2024

                DrawDiv "header-npedido","",""
                    DrawLabel "","",Litpedido
                    DrawSpan "","",trimCodEmpresa(rst("npedido")),""
                CloseDiv
                ...
                DrawDiv "header-nproveedor","",""
                    ...
                    if mode="browse" then
                        ...
                        %><input class="CELDA width20" type="text" size="8" name="nproveedor" value="<%=trimCodEmpresa(iif(nproveedor>"",nproveedor,rst("nproveedor")))%>" onchange="TraerProveedor('add','2');"/>
                        <a class="CELDAREFB" href="javascript:AbrirVentana('../../../compras/proveedores_busqueda.asp?ndoc=pedidos_pro&titulo=<%=LitSelProv%>&mode=search','P',<%=altoventana%>,<%=anchoventana%>)">
                    ...
                    %><input type="hidden" name="ncuentacargo" value="<%=num_cuenta%>"/><%
                    if mode="edit" or mode="add" then
                        %><input class="CELDA width30" type="text" disabled name="nombre" value="<%=nompro%>"/><%
                ...
                if mode="browse" and not rst.EOF then
                    ...
                    if (rst("nfactura")&"")="" and (rst("nalbaran")&"")="" and cstr(cpc)<>"0" then
                        ...
                        <span class='header-note-inputCaja'>
                            <input class="CELDAR7" type="Text" name="impcaja" value="<%=Pendiente%>"/>
                        </span>
                        <span class='header-note-currency'>
                            <font id="fntAbrev" class=ENCABEZADOR7><%=rst("abreviatura")%></font>
                        </span>
                        <span class="header-note-buttonNote">
                            <img id="imgAnotar" src="<%=themeIlion %><%=ImgAnotar%>" <%=ParamImgAnotar%> alt="<%=LitAnotarCaja%>" title="<%=LitAnotarCaja%>" onclick="Acaja('<%=rst("npedido")%>')" style="cursor:pointer"/>
                        </span>
                        <%
                        'rstAux.cursorlocation=3
                ...
                if mode="browse" or mode="save" then
                    if si_tiene_modulo_21 = 0 and si_tiene_modulo_22 = 0 then 
                        DrawDiv "header-resources alignCenter hidden", "", ""
                            %><a class="CELDAREFB" href="javascript:AbrirVentana('../../../central.asp?pag1=./Servicios/recursos.asp&pag2=./Servicios/recursos_bt.asp&codigo=<%=rst("npedido")%>&tipo=pedido a proveedor&viene=enlaces&usuario=<%=session("usuario")%>', 'P', <%=AltoVentana%>, <%=AnchoVentana%>)">&nbsp;&nbsp;&nbsp;<%=LitEnlaces%>&nbsp;&nbsp;&nbsp;</a><%            
                        CloseDiv
                    end if
                    ''ricardo 13-3-20003
                    ...
                    if not rstSelect.EOF then
                        ...
                        if empresa_sup&"">"" then
                            ' Si tiene empresa_sup (central) se comprueba si la central tiene el m�dulo de franquicias y que es franquiciador
                            'dsnCentral=d_lookup("dsn", "clientes", "ncliente='"&empresa_sup&"'", DsnIlion)
                            dsnCentralSelect = "select dsn from clientes with(nolock) where ncliente= ?"
                            dsnCentral = DLookupP1(dsnCentralSelect, empresa_sup&"", adChar, 10, DsnIlion)
                    ...
                    commandAux.ActiveConnection =connAux
                    commandAux.CommandTimeout = 60
                    commandAux.CommandText= "select 'SHLMX_pedidos_pro_impCOMPLETO_FD.asp' as fichero, a.descripcion as descripcion,a.personalizacion,b.parametros as parametros from clientes_formatos_imp as a with(nolock), formatos_imp as b with(nolock) where a.nformato=b.nformato and a.ncliente=? and b.tippdoc='PEDIDO A PROVEEDOR' order by descripcion"                
                    commandAux.CommandType = adCmdText
                    ...
                    if rpc & "">"" then
                        %><label><a id="idPrintFormat" class="CELDAREFB" href='javascript:ComprobarPvp(1,"<%=enc.EncodeForJavascript(npedido)%>","<%=session("ncliente")%>","<%=session("usuario")%>","<%=enc.EncodeForJavascript(novei)%>","","<%=trimCodEmpresa(rst("nproveedor"))%>");' OnMouseOver="self.status='<%=LitImpresionConFormato%>'; return true;" OnMouseOut="self.status=''; return true;"><%=LitImpresionConFormato%></a></label><%
                    else 
                        ...
                        <select class='CELDA' width='150px' style='width:150px' name="formato_impresion">
                            ...
                            while not rstSelect.eof
                                if defecto=rstSelect("descripcion") then
                                    ...
                                    <option selected value="<%=rstSelect("fichero") & "?" & prm%>"><%=rstSelect("descripcion")%></option>
                                <%else
                                    if isnull(rstSelect("parametros")) then
                                        prm=""
                                    else
                    ...
                    'rstSelect.close
                    if not rst.eof then
                        pagina="../../../crearpdf.asp?destinatario=" & rst("nproveedor") & "&ndoc=" & rst("npedido") & "&tdoc=PEDIDO&dedonde=DOCUMENTOC&empresa=" & session("ncliente") & "&mode=DOC&url=compras/"
                        if rpc & "">"" then
                            %><a id="idPrintFormat" class="CELDAREFB hidden" href='javascript:ComprobarPvp(2,"<%=enc.EncodeForJavascript(npedido)%>","<%=session("ncliente")%>","<%=session("usuario")%>","<%=enc.EncodeForJavascript(novei)%>","<%=pagina%>","<%=trimCodEmpresa(rst("nproveedor"))%>");' onmouseover="self.status='<%=LitEnvMail%>'; return true;" onmouseout="self.status=''; return true;"><img src="<%=themeIlion %><%=ImgEnviarEmail%>" <%=ParamImgEnviarEmail%> alt="<%=ucase(LitEnvMail)%>" title="<%=ucase(LitEnvMail)%>"/></a><%
                        else
                            %><a id="idPrintFormat" class="CELDAREFB hidden" href="javascript:AbrirVentana('<%=pagina%>' + document.pedidos_pro.formato_impresion.value + 'npedido=<%="(\'"+enc.EncodeForJavascript(npedido)+"\')"%>','A','<%=AltoVentana+100%>','950')" onmouseover="self.status='<%=LitEnvMail%>'; return true;" onmouseout="self.status=''; return true;"><img src="<%=themeIlion %><%=ImgEnviarEmail%>" <%=ParamImgEnviarEmail%> alt="<%=ucase(LitEnvMail)%>" title="<%=ucase(LitEnvMail)%>"/></a><%
                        end if
                    end if
                    CloseDiv
            ...
            if mode="browse" then
                'BarraOpciones "browse", rst("npedido") 'SHLMX 0515 OCULTAR RECURSOS
            end if
            ActionVersion Altoventana, AnchoVentana
            %><table class ="width100"></table><%

                Alarma "SHLMX_pedidos_pro.asp"%>
            ...
            set commandAux = nothing
            'rstAux.close%>
            <input type="hidden" name="h_nproveedor" value="<%=rst("nproveedor")%>"/>
            <input type="hidden" name="h_npedido" value="<%=rst("npedido")%>"/>
            <input type="hidden" name="h_formapago" value="<%=rst("forma_pago")%>"/>
            <!-- FALLA con enc.EncodeForHtmlAttribute (rst("forma_pago"))-->
            <%if not isnull(rst("nalbaran")) then%>
                <input type="hidden" name="h_nalbaran" value="<%=rst("nalbaran")%>"/>
                <%
                ...
                %>
                <input type="hidden" name="h_nalbaranpro" value="<%=nalbaran_pro%>"/>
            <%else%>
            ...
            <%'enc.EncodeForHtmlAttribute(rst("divisa falla")) %>
                ...
                <div class="Section" id="S_GENERAL_DATA">
                    ...
                    <div class="SectionPanel" id="GENERAL_DATA">
                        <table width="100%" bgcolor="<%=color_blau %>" border="0"><%
                            'DrawFila color_blau
                                if mode="browse" then
                                    ...
                                    EligeCeldaResponsive "text", mode, "CELDA", "", "", 0, LitSerie, "serie", "",trimCodEmpresa(rst("serie"))
                                    'mmg:
                                    set rstMM = Server.CreateObject("ADODB.Recordset")
                                    'rstMM.cursorlocation=3
                                    ...
                                    ' <<< MCA 11/04/05 : A�adir par�metro de usuario con la(s) serie(s) a documentos de compras.
                                    'rstAux.cursorlocation=3
                                ...
                                if mode="add" or mode="edit" then
                                    EligeCelda "check", mode,"CELDA","","",0,LitValorado,"valorado",0,iif(tmp_valorado>"",nz_b(tmp_valorado),rst("valorado"))
                                else 
                                    DrawDiv "1","",""
                                    DrawLabel "","",LitValorado
                                    EligeCeldaResponsive1 "check", mode, "CELDA","","valorado", iif(tmp_valorado>"",nz_b(tmp_valorado),rst("valorado")), ""
                                    CloseDiv
                                end if
                        ...
                        ''response.write("los datos 2 son-" & mode & "-" & datoDivisa & "-" & tmp_divisa & "-" & rst("divisa") & "-<br>")
                                ...
                                if tipo_eligecelda="input" then
                                    if mode="browse" then
                                        ...
                                        EligeCeldaResponsive "text",mode,"CELDA","","",0,LitDivisa,"divisa",5,iif(mode="add",datoDivisa, DLookupP2(divisaSelect, rst("divisa") & "", adVarchar, 15, session("ncliente") & "", adVarchar, 15, session("dsn_cliente")))
                                    else
                                        'd_lookup("abreviatura","divisas","codigo='" & rst("divisa") & "' and codigo like '" & session("ncliente") & "%'",session("dsn_cliente"))
                                        divisaSelect = "select abreviatura from divisas with(nolock) where codigo= ? and codigo like ?+'%'"    
                                        DrawInputCeldaDisabled "","","",5,0,LitDivisa,"divisa",iif(mode="add",datoDivisa, DLookupP2(divisaSelect, rst("divisa") & "", adVarchar, 15, session("ncliente") & "", adVarchar, 15, session("dsn_cliente")))
                                    end if
                        'DrawInputCelda (estilo,ancho,alto,nchar,tabulacion,etiqueta,name,dato)
                                    ...
                                    if mode="add" then
                                        ...
                                        'd_lookup("abreviatura","divisas","codigo='" & rst("divisa") & "' and codigo like '" & session("ncliente") & "%'",session("dsn_cliente")
                                        divisaSelect = "select abreviatura from divisas with(nolock) where codigo= ? and codigo like ?+'%'"
                                        DrawInputCeldaDisabled "","","",5,0,LitDivisa,"divisa", DLookupP2(divisaSelect, rst("divisa") & "", adVarchar, 15,  session("ncliente") & "", adVarchar, 15, session("dsn_cliente"))
                                    end if
                                    connAux.Close
                                    set connAux = nothing
                                    set commandAux = nothing
                        ...
                        ''response.write("los datos 3 son-" & mode & "-" & datoDivisa & "-" & tmp_divisa & "-" & rst("divisa") & "-<br>")
                                %><input type="hidden" name="h_divisa" value="<%=iif(tmp_divisa>"",tmp_divisa,dato_celda)%>"/>
                                <!--<input type="hidden" name="divisafc" value="<%=iif(tmp_divisa>"",tmp_divisa,DIVISA)%>"/>-->
                                <!-- FALLA con el enc.EncodeForHtmlAttribute <input type="hidden" name="divisafc" value="<%=enc.EncodeForHtmlAttribute(iif(mode="add",iif(tmp_divisa>"",tmp_divisa,DIVISA), rst("divisa")))%>"/> -->
                                <input type="hidden" name="divisafc" value="<%=iif(mode="add",iif(tmp_divisa>"",tmp_divisa,DIVISA), rst("divisa"))%>"/><%
                            ...
                            'Informaci�n sobre la moneda base de la empresa.
                            'monedaBase = d_lookup("codigo","divisas","codigo like '" & session("ncliente") & "%' and moneda_base='1'",session("dsn_cliente"))
                            monedaBaseSelect = "select codigo from divisas with(nolock) where codigo like ?+'%' and moneda_base='1'"
                            monedaBase = DLookupP1(monedaBaseSelect, session("ncliente") & "", adVarchar, 15, session("dsn_cliente"))
                    ...
                    'DrawFila "" 'color_blau
                                if mode="browse" then
                                    ...
                                    if  rst("divisa")<>monedaBase then
                                        ...
                                        EligeCeldaResponsive "text", mode, "CELDA", "", "", 0, "", LitFactCambio, "","1" & abrevBase & " = " & CStr(factcambio) & abreviaAtDiv
                                    end if
                                else
                                    ocultar=0  
                                    if mode="add" or mode="edit" then	  
                                    ...
                                    %>
                                        <input type="text" name="nfactcambio" value="<%=CStr(factcambio)%>" size="6" style="text-align:right" onchange="comprobarFactorCambio()"/>
                    ...
                    'DrawFila ""' color_blau
                        ...
                        'pintamos el l�mite alcanzado si es
                            fecha=rst("salida")&""
                            if fecha="" then
                                fecha=rst("fecha")
                            end if
                            alcanzado=gestionalcanzado(mode, fecha,"")
                            %><input type="hidden" name="alcanzado" ID='alcanzado' value="<%=enc.EncodeForHtmlAttribute(alcanzado)%>"/><%
                            'a significa que no ha hay en limite_compras datos para ese mes y a�o
                            if alcanzado<>"a" then
                                if mode="browse" then ' marca 
                                EligeCeldaResponsive "text", mode,"CELDA","","",0,totalcanzado,LitAlcan,0,formatnumber(null_z(alcanzado),n_decimales,-1,0,-1)
                                ...
                                'el mes tiene que tener dos d�gitos porqu� en la bd tiene dos d�gitos sino luego en la select no lo devolver�
                                if len(mes)=1 then
                                    mes="0" & mes
                                end if
                                'JCI
                                ...
                                if not rstLimite.eof then
                                    ...
                                    contrasenya_riesgo_conf=null_s(DLookupP1(contrasenya_riesgo_conf_select, session("ncliente") & "", adChar, 5, session("dsn_cliente")))
                                        ...
                                        %><input type="hidden" name="limite" value="<%=rstLimite("importe")%>"/>
                                        <input type="hidden" name="si_preguntar_riesgo" value="<%=preguntar_riesgo_conf%>"/>
                                        <input type="hidden" name="contrasenya" value="<%=contrasenya_riesgo_conf%>"/>
                        ...
                        if rt = "si" and mode <> "add" then
                            %><td class="CELDA"><a class="CELDAREFB" href="javascript:AbrirVentana('../../../central.asp?pag1=../../../compras/reparto_pedido.asp&pag2=compras/reparto_pedido_bt.asp&mode=browse&ncliente=<%=rst("npedido")%>&ndocumento=<%=rst("nproveedor")%>', 'P', <%=AltoVentana%>, <%=AnchoVentana%>)"><%=LitRepartoPed%></a></td><%
                        end if
                        if esto_se_oculta_ya_que_no_funciona=1 and mode <> "add" then
                            %><td class="CELDA"><a class="CELDAREFB" href="javascript:AbrirVentana('../../../central.asp?pag1=../../../compras/repetir_pedido.asp&pag2=compras/repetir_pedido_bt.asp&mode=browse&ncliente=<%=rst("npedido")%>&ndocumento=<%=enc.EncodeForJavascript(rst("serie"))%>&nproveedor=<%=rst("nproveedor")%>', 'P', <%=AltoVentana%>, <%=AnchoVentana%>)"><%=LitRepetirPed%></a></td><%
                        end if
                    'CloseFila
                    ...
                    CloseDiv
                        ...
                        if mode <> "browse" then
                            ...
                            set commandAux = nothing
                            'rstAux.close
                            %><input type="hidden" name="h_formas_pago" value="<%=defecto%>"/><%
                        else
                            ...
                            EligeCeldaResponsive "text",mode,"CELDA","","",0,LitFormaPago,LitFormaPago,15, DLookupP1(descripFormaPagoSelect, defecto & "", adVarchar, 10, session("dsn_cliente"))
                        end if
                        ...
                        if mode <> "browse" then
                            ...
                            EligeCeldaResponsive "text",mode,"CELDA","","",0,LitTipoPago,LitTipoPago,15,DLookupP1(tipoPagoSelect, defecto & "", adVarchar, 8, session("dsn_cliente"))
                        end if
                    'CloseFila
                    ...
                    'DrawFila "" 'color_blau
                            ...
                            if mode="browse" then
                                ...
                                EligeCeldaResponsive "text", mode,"CELDA","","",0,LitNumAlbaran,LitNumAlbaran,0,DLookupP1(nalbaranSelect, iif(tmp_nalbaran>"",tmp_nalbaran,iif(isnull(rst("nalbaran")),"",rst("nalbaran"))) & "", adVarchar, 20, session("dsn_cliente"))
                            else
                            %><div class="col-lg-4 col-md-6 col-sm-6 col-xs-12"><%
                                ...
                                %><input class='width60' type="text" name="nalbaran" value="<%=DLookupP1(nalbaranProSelect, iif(tmp_nalbaran>"",tmp_nalbaran,iif(isnull(rst("nalbaran")),"",rst("nalbaran"))) & "", adVarchar, 20, session("dsn_cliente"))%>" disabled="disabled"/><%
                            %></div><%
                            end if
                            'if mode<>"browse" then DrawCelda "CELDA","10","",0,"��"
                            ...
                            if mode="browse" then
                                ...
                                EligeCeldaResponsive "text", mode,"CELDA","","",0,LitNumFactura,LitNumFactura,0,DLookupP1(nfacturaProSelect, iif(tmp_nfactura>"",tmp_nfactura,iif(isnull(rst("nfactura")),"",rst("nfactura"))) & "", adVarchar, 20, session("dsn_cliente"))
                            else
                                %><div class="col-lg-4 col-md-6 col-sm-6 col-xs-12"><%
                                    %><label><%=LitNumFactura%></label><%
                                        ...
                                        nfacturaProSelect = "select nfactura_pro from facturas_pro with(nolock) where nfactura= ?"
                                    %><input class='width60' type="text" name="nfactura" value="<%=DLookupP1(nfacturaProSelect, iif(tmp_nfactura>"",tmp_nfactura,iif(isnull(rst("nfactura")),"",rst("nfactura"))) & "", adVarchar, 20, session("dsn_cliente"))%>" disabled="disabled"/><%
                                %></div><%
                        end if
                    'CloseFila
                    'DrawFila "" 'color_blau
                        if si_tiene_modulo_proyectos<>0 then
                            ...
                            if mode <> "browse" then
                                %><div class="col-lg-4 col-md-6 col-sm-6 col-xs-12"><%
                                    ...
                                    %><input class="width60" type="hidden" name="cod_proyecto" value="<%=enc.EncodeForHtmlAttribute(iif(tmp_cod_proyecto>"",tmp_cod_proyecto,iif(isnull(rst("cod_proyecto")),"",rst("cod_proyecto"))))%>"/><%
                                ...
                                EligeCeldaResponsive "text", mode,"CELDA","","",0,LitProyecto,LitProyecto,0,DLookupP1(nombreProyectoSelect, rst("cod_proyecto") & "", adVarchar, 15, session("dsn_cliente"))
                            end if
                            'if mode<>"browse" then DrawCelda "CELDA","10","",0,"��"
                        end if
                        ...
                        if mode <> "browse" then
                            DrawDiv "1","",""
                            DrawLabel "","",LitFechaEntrega%><input class="CELDA" type="text" name="fecha_entrega" value="<%=enc.EncodeForHtmlAttribute(iif(tmp_fecha_entrega>"",tmp_fecha_entrega,iif(isnull(rst("fecha_entrega")),"",rst("fecha_entrega"))))%>"/><%
                            DrawCalendar "fecha_entrega"
                            CloseDiv
                        else
                            EligeCeldaResponsive "text",mode,"CELDA","","",0,LitFechaEntrega,LitFechaEntrega,0,rst("fecha_entrega")
                        ...
                        'DrawFila "" 'color_blau
                            ...
                            if mid(rst("edi"),6,5)=LitEmpCovaldroper then
                                ...
                                %><div class="col-lg-4 col-md-6 col-sm-6 col-xs-12" id="redi"><%
                                    %><label><%=LitEDI%></label><%
                                    %><a class='CELDAREFB' href=javascript:pedir("<%=enc.EncodeForJavascript(npedido)%>")><%=rst("edi")%></a><%
                                %></div><% 
                            else
                                if mode="first_save" then
                                    EligeCelda "", mode,"CELDA","","",0,LitEDI,"nfactura",0,rst("edi")
                                else 
                                    EligeCeldaResponsive "text", mode,"CELDA","","",0,LitEdi,LitEdi,0,rst("edi")
                                end if
                    ...
                    'DrawFila "" 'color_blau
                        ...
                        if mode="browse" then
                            %><div class="col-lg-4 col-md-6 col-sm-6 col-xs-12"><%
                                    ...
                                    if rst("incoterms")<>"" then
                                        %><a class='CELDAREFB' href=javascript:pedir("<%=enc.EncodeForJavascript(npedido)%>")><%=enc.EncodeForHtmlAttribute(rst("incoterms"))&""%></a><%
                                    else
                                        DrawSpan "CELDA","","",""
                                    end if
                            %></div><%
                        ...
                        if mode="browse" then
                            ...
                            %><div class="col-lg-4 col-md-6 col-sm-6 col-xs-12"><%
                                    ...
                                    if rst("fob")<>"" then
                                        %><a class='CELDAREFB' href=javascript:pedir("<%=enc.EncodeForJavascript(npedido)%>")><%=enc.EncodeForHtmlAttribute(rst("fob"))&""%></a><%
                                    else 
                                        DrawSpan "CELDA","","",""
                                    end if%></div><%
                        else
                            %><div class="col-lg-4 col-md-6 col-sm-6 col-xs-12"><%
                                ...
                                %><input class="width60"  type="text"  name="fob" value="<%=enc.EncodeForHtmlAttribute(defecto)%>"/></div><%
                        end if
                    'CloseFila

                    'btorreme - Se añade combo de tiendas 31/01/2024' - Se agrega validación para edición con user admin
                        if mode <> "browse" then
                            if mode = "edit" and session("user_admin")&"" = "0" then
                                %><div class="col-lg-4 col-md-6 col-sm-6 col-xs-12"><% 
                                    DrawLabel "","","Centro de costos"
                                    DrawSpan "","",d_lookup("descripcion","tiendas","codigo='" & rst("campo01") & "'",session("dsn_cliente")),""
                                %></div><%
                            else
                                defecto=iif(tmp_tienda>"",tmp_tienda,rst("campo01"))
                                rstAux.cursorlocation=3
                                rstAux.Open "EXEC MXPRY_GetStoresByEmployed @PI_NCOMPANY = '" & session("ncliente") & "', @PI_vUserCode = '" & session("ncliente") + session("usuario") & "'", dsn_shlmxproy 'Cargado de tiendas por usuario
                                DrawSelectCelda "CELDALEFT","","",0,"Centro de costos","tienda",rstAux,defecto,"codigo","descripcion","onchange","javascript:actualizarHTienda(this);"
                                rstAux.close
                            end if
			            else
                            EligeCeldaResponsive "text", mode, "CELDA", "", "", 0, "Centro de costos", "Centro de costos", "",d_lookup("descripcion","tiendas","codigo='" & rst("campo01") & "'",session("dsn_cliente"))
				        end if
                    'btorreme - Se añade combo de tiendas 31/01/2024'
                    ...
                    'DrawFila "" 'color_blau
                        ...
                        if mode="browse" then
                            ...
                            %><div class="col-lg-4 col-md-6 col-sm-6 col-xs-12"><%
                                    ...
                                    if rst("portes") <> "" then
                                        %><a class='CELDAREFB' href=javascript:pedir("<%=enc.EncodeForJavascript(npedido)%>")><%=enc.EncodeForHtmlAttribute(rst("portes"))&""%> </a><%
                                    else
                                        DrawSpan "CELDA","","",""
                                    end if %></div><%
                        else
                    ...
                    'DrawFila "" 'color_blau
                        ...
                        if mode="browse" then
                            ...
                            %><div class="col-lg-4 col-md-6 col-sm-6 col-xs-12"><%
                                    ...
                                    if rst("salida")<>"" then
                                        %><a class='CELDAREFB' href=javascript:pedir('<%=enc.EncodeForJavascript(npedido)%>') ><%=enc.EncodeForHtmlAttribute(rst("salida"))&""%> </a><%
                                    else 
                                        DrawSpan "CELDA","","",""
                                    end if%></div><%
                        else
                            ...
                            defecto=iif(tmp_salida>"",tmp_salida,iif(rst("salida")>"",rst("salida"),""))%><input class="CELDA" maxlength="10" type="text" name="salida" value="<%=enc.EncodeForHtmlAttribute(defecto)%>"/><%
                            DrawCalendar "salida"
                            CloseDiv
                            if mode="add" then
                                %><script language=javascript type="text/javascript">
                    ...
                    'DrawFila "" 'color_blau
                        ...
                        if mode="browse" then
                            ...
                            DrawSpan "CELDA","",pintar_saltos_espacios(rst("observaciones")&""), ""
                        else
                    ...
                    DrawDiv "1","",""
                        ...
                        if mode="browse" then
                            ...
                            DrawSpan "CELDA","",pintar_saltos_espacios(rst("notas")&""), ""
                        else
                            'DrawTextCeldaSpan "CELDA","","",2,100,"","notas",iif(rst("notas")>"",rst("notas"),""),3
                            DrawTextarea "width60","","notas",iif(rst("notas")>"",rst("notas"),""),""
                        end if
                    CloseDiv
                                    ...
                                    while not rstAux2.eof
                                        ...
                                        if rstAux2("titulo") & "">"" then
                                            ...
                                            if rstAux2("tipo")=1 then
                                                ...
                                                DrawDiv "1","",""
                                                DrawLabel "","",rstAux2("titulo") & ":"%><input type="text" style="width:155px;" class="CELDA" name="<%="campo" & num_campo%>"  maxlength="<%=tamany%>" value="<%=valor_campo_perso%>" />
                                                <%CloseDiv
                                            ...
                                            elseif rstAux2("tipo")=4 then
                                                ...
                                                DrawDiv "1","",""
                                                DrawLabel "","",rstAux2("titulo") & ":"%><input type="text" class="CELDA" name="<%="campo" & num_campo%>" style="width:155px;" maxlength="<%=tamany%>" value="<%=valor_campo_perso%>"/>
                                                <%CloseDiv
		...
		<%if mode="browse" or mode ="edit" then
 			'** Campo oculto para controlar si el pedido est� facturado.
            'rstAux.cursorlocation=3
			...
			if not isnull(rstAux("nfactura")) then
				%><input type="hidden" name="h_nfactura" value="<%=rstAux("nfactura")%>"/><%
				...
				%><input type="hidden" name="h_nfacturapro" value="<%=nfactura_pro%>"/><%
			else
                    ...
                    <div class="SectionPanel" id="FINANCIAL_DATA" style="<%=iif(mode="add" or mode="edit","display: none","display: ")%>">  
                        <div id="tabs" style="display:none">
                            <ul>    
                                ...
                                <li id="li-concepts" class="hidden"><a href="#tabs-concepts"><%=LitConceptos %></a></li>
                                <%if vencesp="1" then%>
                            ...
                            'Mostrar los Detalles del pedido.
                                %><div id="tabs-details" class="overflowXauto">
                                    ...
                                    pagina="../../../central.asp?pag1=SHLMX_pedidos_prodet.asp&ndoc=" +rst("npedido") + "&nproveedor=" + rst("nproveedor") + "&mode=add&pag2=SHLMX_pedidos_prodet_bt.asp&titulo=" & LitDetallesPedido & " " & rst("npedido")
                                    %>
                                    <!-- GPD (05/03/2007) -->
                                    <table width="835" border='0' cellspacing="1" cellpadding="1">
                                        <% DrawFila "" %>
                                            <td width="100">
                                                ...
                                                <a class="CELDAREFB" href="javascript:mostrarCondicionesCompra('<%=rst("nproveedor")%>');"><%=LitCondCompra%></a>		   		        
                                                <%end if%>
                                            </td>		   		    
                                        </tr>
                                    </table>
                                    <table class="width90 md-table-responsive bCollapse">
                                        <%DrawFila color_terra
			                                ...
			                                DrawCeldaDet "'CELDAL7 underOrange width10'","","",0,"ID Producto Proveedor"
			                                DrawCeldaDet "'CELDAL7 underOrange width15'","","",0,"Nombre"
			                                ...
			                                DrawCeldaDet "'CELDAL7 underOrange width5'","","",0,"Costo"
			                                DrawCeldaDet "'CELDAL7 underOrange width5'","","",0,"IEPS %"
			                                DrawCeldaDet "'CELDAL7 underOrange width5'","","",0,"IEPS $"
			                                DrawCeldaDet "'CELDAL7 underOrange width5'","","",0,"Tipo Dto"
			                                ...
			                                DrawCeldaDet "'CELDAL7 underOrange width5'","","",0,(LitIva & " $")
			                                DrawCeldaDet "'CELDAL7 underOrange width5'","","",0,"Subtotal"
                                    ...
                                    <%if isnull(rst("nfactura")) and isnull(rst("nalbaran")) AND NOT blnEstadoCierre then
                                        %><iframe id='frDetallesIns'  class="width90 iframe-input md-table-responsive" name='fr_DetallesIns' src='SHLMX_pedidos_prodetins.asp?ndoc=<%=rst("npedido")%>&nproveedor=<%=rst("nproveedor")%>&modp=<%=modP%>&almacenSerie=<%=almacenSerie %>&almacenTPV=<%=almacenTPV %>&tienda=<%=rst("campo01")%>' frameborder="no" scrolling="no" noresize="noresize"></iframe><%
                                    end if
                                    %>
                                    <iframe id='frDetalles' class="width90 md-table-responsive" name="fr_Detalles" src='SHLMX_pedidos_prodet.asp?ndoc=<%=rst("npedido")%>&nproveedor=<%=rst("nproveedor")%>&modp=<%=modP%>&almacenSerie=<%=almacenSerie %>&almacenTPV=<%=almacenTPV %>&EstadoCierre=<%=blnEstadoCierre %>' height='150' frameborder="yes" noresize="noresize"></iframe>
                            ...
                            <div id="tabs-concepts" class="overflowXauto">
                                ...
                                pagina="../../../central.asp?pag1=./compras/pedidos_procon.asp&ndoc=" + npedido + "&nproveedor=" + nproveedor + "&mode=add&pag2=./compras/pedidos_procon_bt.asp&titulo=" &  LitConceptoPedido & " "& rst("npedido")
                                %>
                                <table class="width90 md-table-responsive bCollapse">
                                    <%'Fila de encabezado
                                    DrawFila color_terra
                                        ...
                                        DrawCeldaDet "'CELDAL7 underOrange width5'","","",0,(LitPVP & " + IEPS")
                                        DrawCeldaDet "'CELDAL7 underOrange width5'","","",0,LitDto
                                        DrawCeldaDet "'CELDAL7 underOrange width5'","","",0,LitIva
                                    ...
                                    CloseFila
                                        ...
                                        'Linea de inserci�n de un detalle
                                        DrawFila color_blau%>
                                    ...
                                    <iframe id="frConceptos" name="fr_Conceptos" class="width90 md-table-responsive" src='../../../compras/pedidos_procon.asp?mode=browse&ndoc=<%=rst("npedido")%>' height='80' frameborder="yes" noresize="noresize"></iframe>
                                </div>
                                <%if vencesp="1" then%>
                                    <div id="tabs-vencimientos">
                                        <table class="width90 md-table-responsive bCollapse"><%
                                            'Fila de encabezado
                                                ...
                                                if isnull(rst("nfactura")) and isnull(rst("nalbaran")) then
                                                    DrawFila color_blau
                                                        ...
                                                        <td class="CELDAL7 underOrange width20" >
                                                            <input class="CELDAR7 width100" type="text" name="tantoVto" value="0" onchange="javascript:RoundNumValue(this,<%=NdecDiPedido%>);"/>
                                                        </td>
                                                        <td class="CELDAL7 underOrange width20" >
                                                            <a class="ic-accept noMTop" href="javascript:addVencimiento('<%=rst("npedido")%>');" onblur="javascript:document.pedidos_pro.fechaVto.focus();">
                                                                <img src="<%=themeIlion %><%=ImgNuevo%>" <%=ParamImgNuevo%> alt="<%=LitNuevo%>" title="<%=LitNuevo%>"/></a>
                                                        </td><%
                                                    CloseFila
                                                end if
                                            ...
                                            %><iframe id="frVencimientos" name="fr_Vencimientos" class="width90 md-table-responsive" src='vencimientos_pro_config.asp?mode=browse&tdocumento=pedidos_pro&ndoc=<%=rst("npedido")%>' style="width:355px;height:200px;" frameborder="yes" noresize="noresize"></iframe>
                                    </div>
                                <%end if%>
                            <div id="tabs-payments" class="overflowXauto">
                                ...
                                <%CloseFila
                                if isnull(rst("nfactura")) and isnull(rst("nalbaran")) then
                                    'Linea de inserci�n de un pago a cuenta
                                    DrawFila color_blau%>
                                        ...
                                        <td class="CELDAR7 underOrange width5">
                                            <input class="CELDAR7 width100" type="text" name="importePago" value="0" onchange="RoundNumValue(this,<%=NdecDiPedido%>);importepagoComp();"/>
                                        </td>
                                        <%
                                        'rstSelect.cursorlocation=3
                                ...
                                <iframe id="frPagosCuenta" name="fr_PagosCuenta"  class="width90 md-table-responsive" src='../../../compras/pedidos_propagos.asp?mode=browse&ndoc=<%=rst("npedido")%>' height='80' frameborder="yes" noresize="noresize"></iframe>
                            </div>
                            <div id="tabs-pedcli" class="overflowXauto" >
                                ...
                                <iframe id="frPedcli" name="fr_Pedcli" class="width90 md-table-responsive" src='../../../compras/pedidos_propedcli.asp?mode=browse&ndoc=<%=rst("npedido")%>' width='600' height='120' frameborder="yes" noresize="noresize"></iframe>
                            </div>
                            <div id="tabs-send">
                                <%if mode="browse" then
                                    if rst("dir_envio")>"" then
                                        pagina="../../../central.asp?pag1=Custom/ShellMexico/compras/SHLMX_pedidos_prodireccion_env.asp&ndoc=" &rst("npedido") & "&mode=browse&pag2=Custom/ShellMexico/compras/SHLMX_pedidos_prodireccion_env_bt.asp&titulo=" & ucase(LitDatosEnvio) & " " & trimCodEmpresa(rst("npedido"))
                                    else
                                        pagina="../../../central.asp?pag1=Custom/ShellMexico/compras/SHLMX_pedidos_prodireccion_env.asp&ndoc=" &rst("npedido") & "&mode=edit&pag2=Custom/ShellMexico/compras/SHLMX_pedidos_prodireccion_env_bt.asp&titulo=" & ucase(LitDatosEnvio) & " " & trimCodEmpresa(rst("npedido"))
                                    end if
        ...
        <% ''enc.EncodeForHtmlAttribute(rst("descuento")) y enc.EncodeForHtmlAttribute(rst("descuento2")) falla la p�gina. %>
        <input type="hidden" name="desc1" value="<%=rst("descuento")%>"/>
		<input type="hidden" name="desc2" value="<%=rst("descuento2")%>"/>
        ...
        <div class="Section" id="S_TOTAL">
            ...
            <div class="SectionPanel" id="DATTOTAL"><%
                ...
                DrawDiv "4", "", ""
                    ...
                    DrawCelda "ENCABEZADOL","","",0, DLookupP1(abrevDivSelect, session("ncliente") & "01" &"", adVarchar, 15, session("dsn_cliente"))

                    CloseDiv%>
                    <%
                    xxxcantidad	= null_z(d_sum("cantidad","detalles_ped_pro","npedido='" & rst("npedido") & "'",session("dsn_cliente")))
                    xxxcantidadcon	= null_z(d_sum("cantidad","conceptos_ped_pro","npedido='" & rst("npedido") & "'",session("dsn_cliente")))
                    xxxcantidad = xxxcantidad + xxxcantidadcon
                    DrawDiv "4", "", ""
                        DrawLabel "", "",LitCantidad
                        EligeCeldaResponsive1 "input", mode,"ENCABEZADOR", "","cantidad",formatnumber(null_z(xxxcantidad),n_decimales,-1,0,iif(mode="browse",-1,0)), iif(mode="browse","id='cantidad'","disabled")
                    CloseDiv%>
                    <%DrawDiv "4", "", ""
                        DrawLabel "", "",LitBruto
                        EligeCeldaResponsive1 "input", mode,"ENCABEZADOR", "","importe_bruto",formatnumber(null_z(rst("importe_bruto")),n_decimales,-1,0,iif(mode="browse",-1,0)), iif(mode="browse","id='importe_bruto'","disabled")
                    CloseDiv%>
                    <%DrawDiv "4", "", ""
                        DrawLabel "", "",LitDto
                        if mode<>"browse" then
                            %><input class="ENCABEZADOR" type="text" name="dto" value="<%=iif(tmp_descuento>0,tmp_descuento,iif(rst("descuento")>"",rst("descuento"),0))%>" onchange="RoundNumValue(this,<%=DECPOR%>);Recalcula('<%=total_iva_bruto%>','<%=total_re_bruto%>');" <%=iif(countTotalDetail > 0,"disabled","")%> /><% 'btorreme - Se agrega disabled según detalle
                        else
                            EligeCeldaResponsive1 "input", mode,"ENCABEZADOR","","dto", cstr(formatnumber(null_z(rst("descuento")),decpor,-1,0,iif(mode="browse",-1,0))) + "%", "id='dto'"
                        end if
                    CloseDiv%>
                    <%DrawDiv "4", "", ""
                        DrawLabel "", "",LitDto2
                        if mode<>"browse" then
                            %><input class="ENCABEZADOR" type="text" name="dto2" value="<%=iif(tmp_descuento2>0,tmp_descuento2,iif(rst("descuento2")>"",rst("descuento2"),0))%>" onchange="RoundNumValue(this,<%=DECPOR%>);Recalcula('<%=total_iva_bruto%>','<%=total_re_bruto%>');" <%=iif(countTotalDetail > 0,"disabled","")%> /><% 'btorreme - Se agrega disabled según detalle
                        else
                    ...
                    <%DrawDiv "4", "", ""
                        ...
                        EligeCeldaResponsive1 "input", mode,"ENCABEZADOR","","total_descuento",formatnumber(null_z(rst("total_descuento")),n_decimales,-1,0,iif(mode="browse",-1,0)),iif(mode="browse","id='total_descuento'","disabled")
                        %><input type="hidden" name="h_total_descuento" value="<%=rst("total_descuento")%>"/><%
                    CloseDiv%>
                    <%DrawDiv "4", "", ""
                        DrawLabel "", "",LitImponible
                        EligeCeldaResponsive1 "input", mode,"ENCABEZADOR","","base_imponible",formatnumber(null_z(rst("base_imponible")),n_decimales,-1,0,iif(mode="browse",-1,0)),iif(mode="browse","id='base_imponible'","disabled")
                        %><input type="hidden" name="h_base_imponible" value="<%=rst("base_imponible")%>"/><%
                    CloseDiv%>
                    <%DrawDiv "4", "", ""
                        DrawLabel "", "",LitTotalIva
                        EligeCeldaResponsive1 "input", mode,"ENCABEZADOR","","total_iva",cstr(formatnumber(Null_z(rst("total_iva")),n_decimales,-1,0,iif(mode="browse",-1,0))),iif(mode="browse","id='total_iva'","disabled")
                        %><input type="hidden" name="h_total_iva" value="<%=rst("total_iva")%>"/><%
                    CloseDiv%>
                    <%DrawDiv "4", "", ""
                        DrawLabel "", "",LitRe
                        EligeCeldaResponsive1 "input", mode,"ENCABEZADOR","","total_re",cstr(formatnumber(Null_z(rst("total_re")),n_decimales,-1,0,iif(mode="browse",-1,0))),iif(mode="browse","id='total_re'","disabled")
                    ...
                    <%if ((rst("recargo")<>0) or mode="edit" or mode="add") then%>
                        ...
                        if mode<>"browse" then
                            %><input class="ENCABEZADOR" type="text" name="recargo" value="<%=enc.EncodeForHtmlAttribute(iif(tmp_recargo>0,tmp_recargo,iif(rst("recargo")>"",rst("recargo"),0)))%>" onchange="RoundNumValue(this,<%=DECPOR%>);Recalcula('<%=total_iva_bruto%>','<%=total_re_bruto%>');"/><%
                        else
                            EligeCeldaResponsive1 "input", mode,"ENCABEZADOR","","recargo",formatnumber(null_z(rst("recargo")),2,-1,0,iif(mode="browse",-1,0)) + "%","id='recargo'"
                        end if
                        CloseDiv%>

                        <%DrawDiv "4", "", ""
                            DrawLabel "", "",LitTotalRecargo
                            EligeCeldaResponsive1 "input", mode,"ENCABEZADOR","","total_recargo",formatnumber(null_z(rst("total_recargo")),n_decimales,-1,0,iif(mode="browse",-1,0)),iif(mode="browse","id='total_recargo'","disabled")
                    ...
                    if ((rst("irpf")<>0) or mode="edit" or mode="add") then%>
                        ...
                        <%DrawDiv "4", "", ""
                            DrawLabel "", "",Litirpf
                            if mode<>"browse" then
                                %><input class="ENCABEZADOR" type="text" name="irpf" value="<%=enc.EncodeForHtmlAttribute(iif(tmp_irpf>0,tmp_irpf,iif(rst("irpf")>"",rst("irpf"),0)))%>" onchange="RoundNumValue(this,<%=DECPOR%>);Recalcula('<%=total_iva_bruto%>','<%=total_re_bruto%>');"/><%
                            else
                                EligeCeldaResponsive1 "input", mode,"ENCABEZADOR","","irpf",formatnumber(null_z(rst("irpf")),2,-1,0,iif(mode="browse",-1,0)) + "%","id='irpf'"
                            end if
                        CloseDiv%>

                        <%DrawDiv "4", "", ""
                            DrawLabel "", "",LitTotalirpf
                            EligeCeldaResponsive1 "input", mode,"ENCABEZADOR","","total_irpf",formatnumber(null_z(rst("total_irpf")),n_decimales,-1,0,iif(mode="browse",-1,0)),iif(mode="browse","id='total_irpf'","disabled")
                            %><input type="hidden" name="h_total_irpf" value="<%=rst("total_irpf")%>"/><%
                        CloseDiv%>
                    <%end if%>
                    ...
                    <%DrawDiv "4", "", ""
                        DrawLabel "", "",LitTotal
                        EligeCeldaResponsive1 "input", mode,"ENCABEZADOR","","total_pedido",formatnumber(null_z(rst("total_pedido")),n_decimales,-1,0,iif(mode="browse",-1,0)),iif(mode="browse","id='total_pedido'","disabled")
                        %><input type="hidden" name="h_total_pedido" value="<%=rst("total_pedido")%>"/><%
                    CloseDiv%>
                ...
                if DLookupP1(imp_equiv_select, session("ncliente") & "", adChar, 5, session("dsn_cliente")) then
                    ...
                    DIVISA=iif(tmp_divisa>"",tmp_divisa,iif(mode="add", DLookupP1(codigoDivisaSelect, session("ncliente") & "", adVarchar, 15, session("dsn_cliente")) ,rst("divisa")))
                        ...
                        DrawCelda "ENCABEZADOL","","",0, DLookupP1(abrevDivSelect,  session("ncliente") & "", adVarchar, 15, session("dsn_cliente"))
                    ...
                    EligeCelda "input", mode,iif(mode="browse","ENCABEZADOR id='Pimporte_bruto'","ENCABEZADOR disabled"),"","",0,"","Pimporte_bruto",10,formatnumber(CambioDivisa(null_z(rst("importe_bruto")),DIVISA,session("ncliente") & "01"), DLookupP1 (ndecDivSelect, session("ncliente"), adVarchar, 15, session("dsn_cliente")),-1,0,iif(mode="browse",-1,0))
                    EligeCelda "input", mode,"ENCABEZADOR disabled id='Pdescuento'","","",0,"","Pdescuento",3,formatnumber(null_z(rst("descuento")),decpor,-1,0,iif(mode="browse",-1,0))
                    EligeCelda "input", mode,"ENCABEZADOR disabled id='Pdescuento2'","","",0,"","Pdescuento2",3,formatnumber(null_z(rst("descuento2")),decpor,-1,0,iif(mode="browse",-1,0))
                    'd_lookup("ndecimales","divisas","codigo='" & session("ncliente") & "01'",session("dsn_cliente"))
                    ndecDivSelect = "select ndecimales from divisas with(nolock) where codigo= ?+'01'"
                    EligeCelda "input", mode,iif(mode="browse","ENCABEZADOR id='Ptotal_descuento'","ENCABEZADOR disabled"),"","",0,"","Ptotal_descuento",10,formatnumber(CambioDivisa(null_z(rst("total_descuento")),DIVISA,session("ncliente") & "01"), DLookupP1 (ndecDivSelect, session("ncliente"), adVarchar, 15, session("dsn_cliente")),-1,0,iif(mode="browse",-1,0))
                    EligeCelda "input", mode,iif(mode="browse","ENCABEZADOR id='Pbase_imponible'","ENCABEZADOR disabled"),"","",0,"","Pbase_imponible",10,formatnumber(CambioDivisa(null_z(rst("base_imponible")),DIVISA,session("ncliente") & "01"), DLookupP1 (ndecDivSelect, session("ncliente"), adVarchar, 15, session("dsn_cliente")),-1,0,iif(mode="browse",-1,0))
                    EligeCelda "input", mode,iif(mode="browse","ENCABEZADOR id='Ptotal_iva'","ENCABEZADOR disabled"),"","",0,"","Ptotal_iva",10,formatnumber(CambioDivisa(null_z(rst("total_iva")),DIVISA,session("ncliente") & "01"), DLookupP1 (ndecDivSelect, session("ncliente"), adVarchar, 15, session("dsn_cliente")),-1,0,iif(mode="browse",-1,0))
                    EligeCelda "input", mode,iif(mode="browse","ENCABEZADOR id='Ptotal_re'","ENCABEZADOR disabled"),"","",0,"","Ptotal_re",10,formatnumber(CambioDivisa(null_z(rst("total_re")),DIVISA,session("ncliente") & "01"), DLookupP1 (ndecDivSelect, session("ncliente"), adVarchar, 15, session("dsn_cliente")),-1,0,iif(mode="browse",-1,0))
                    if ((rst("recargo")<>0) or mode="edit" or mode="add") then
                        EligeCelda "input", mode,"ENCABEZADOR disabled id='Precargo'","","",0,"","Precargo",3,formatnumber(null_z(rst("recargo")),2,-1,0,iif(mode="browse",-1,0))
                        EligeCelda "input", mode,iif(mode="browse","ENCABEZADOR id='Ptotal_recargo'","ENCABEZADOR disabled"),"","",0,"","Ptotal_recargo",10,formatnumber(CambioDivisa(null_z(rst("total_recargo")),DIVISA,session("ncliente") & "01"), DLookupP1 (ndecDivSelect, session("ncliente"), adVarchar, 15, session("dsn_cliente")),-1,0,iif(mode="browse",-1,0))
                    end if
                    if ((rst("irpf")<>0) or mode="edit" or mode="add") then
                        EligeCelda "input", mode,"ENCABEZADOR disabled id='Pirpf'","","",0,"","Pirpf",3,formatnumber(null_z(rst("irpf")),2,-1,0,iif(mode="browse",-1,0))
                        EligeCelda "input", mode,iif(mode="browse","ENCABEZADOR id='Ptotal_irpf'","ENCABEZADOR disabled"),"","",0,"","Ptotal_irpf",10,formatnumber(CambioDivisa(null_z(rst("total_irpf")),DIVISA,session("ncliente") & "01"), DLookupP1 (ndecDivSelect, session("ncliente"), adVarchar, 15, session("dsn_cliente")),-1,0,iif(mode="browse",-1,0))
                    end if
                    EligeCelda "input", mode,iif(mode="browse","ENCABEZADOR id='Ptotal_pedido'","ENCABEZADOR disabled"),"","",0,"","Ptotal_pedido",10,formatnumber(CambioDivisa(null_z(rst("total_pedido")),DIVISA,session("ncliente") & "01"), DLookupP1 (ndecDivSelect, session("ncliente"), adVarchar, 15, session("dsn_cliente")),-1,0,iif(mode="browse",-1,0))
    ...
    elseif rst.EOF then%>
        <script language="javascript" type="text/javascript">
            parent.botones.document.location = "SHLMX_pedidos_pro_bt.asp?mode=search";
        </script>
    <%end if
